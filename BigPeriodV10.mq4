...
int MagicNumberOne = 1000;
int MagicNumberTwo = 2000;
int MagicNumberThree = 3000;
int MagicNumberFour = 4000;
int MagicNumberFive = 5000;
int MagicNumberSix = 6000;
int MagicNumberSeven = 7000;
int MagicNumberEight = 8000;

int MagicNumberNine = 9000;
int MagicNumberTen = 10000;
int MagicNumberEleven = 11000;
int MagicNumberTwelve = 12000;



int MagicNumberThirteen = 13000;
int MagicNumberFourteen = 14000;
int MagicNumberFifteen = 15000;
int MagicNumberSixteen = 16000;
...
void initmagicnumber()
{
	MagicNumberOne = 1000;
	MagicNumberTwo = 2000;
	MagicNumberThree = 3000;
	MagicNumberFour = 4000;
	MagicNumberFive = 5000;
	MagicNumberSix = 6000;
	MagicNumberSeven = 7000;
	MagicNumberEight = 8000;
	MagicNumberNine = 9000;
	MagicNumberTen = 10000;
	MagicNumberEleven = 11000;
	MagicNumberTwelve = 12000;
	MagicNumberThirteen = 13000;
	MagicNumberFourteen = 14000;
	MagicNumberFifteen = 15000;
	MagicNumberSixteen = 16000;	
}
...
int MakeMagic(int SymPos,int Magic)
{
   ...
   symbolvalue = SymPos*100000 + Magic;
   return symbolvalue;
}
...
void initglobaltradeflag()
{
  ...
  timelocal = TimeCurrent() + globaltimezonediff*60*60;
	...
	gstartday = TimeDay(timelocal);
	gstarthour = TimeHour(timelocal);
	gstartminute = TimeMinute(timelocal);
...
void initglobalamount()
{
  ...
  timelocal = TimeCurrent() + globaltimezonediff*60*60;
	...
	else if((AccountBalance() > 200)&&(AccountBalance() <= 300))
	{
		MyLotsH          =0.02;
		MyLotsL          =0.02;							
	}
	...
	else if((AccountBalance() > 900)&&(AccountBalance() <= 1000))
	{
		MyLotsH          =0.09;
		MyLotsL          =0.09;	
							
	}		
	else if((AccountBalance() > 1000)&&(AccountBalance() <= 4000))
	{
		MyLotsH          =0.1;
		MyLotsL          =0.1;	
	}		
	else if((AccountBalance() > 4000)&&(AccountBalance() <= 8000))
	{
		MyLotsH          =0.2;
		MyLotsL          =0.2;	
	}		
	...
	else if((AccountBalance() > 10000)&&(AccountBalance() <= 50000))
	{
		MyLotsH          =0.5;
		MyLotsL          =0.5;	
	}		
	...
	//大周期交易，缩小交易手数为小周期的1/4
	MyLotsH          =MyLotsH/4;
	MyLotsL          =MyLotsL/4;	
		
	MyLotsH = NormalizeDouble(MyLotsH,2);		
	MyLotsL = NormalizeDouble(MyLotsL,2);		
		
	//基于不拒绝交易的原则，确定最低交易手数为 0.01
	if((MyLotsH<0.009)||(MyLotsL<0.009))
	{
		MyLotsH          =0.01;
		MyLotsL          =0.01; 		
	}

	Print("Big Period init Amount is = "+MyLotsH+":"+MyLotsL);	 		
...
void autoadjustglobalamount()
{
  ...
  timelocal = TimeCurrent() + globaltimezonediff*60*60;
	...
	Print("Big Period Auto adjest Amount is = "+MyLotsH+":"+MyLotsL);	 		
...
/*非Openday期间不新开单，规避停盘期间买卖风险*/
/*隔夜费用也放在这个函数里实现，考虑长期持有，因此只交易隔夜费用为正的货币对！*/
...
bool isvalidmagicnumber(int magicnumber)
{
	...
	SymPos = ((int)magicnumber) /100000;
	NowMagicNumber = magicnumber - SymPos *100000;
	...
	NowMagicNumber = ((int)NowMagicNumber) /1000;
	...
	for (i = 0; i < OrdersTotal(); i++)
	{
		if (OrderSelect(i,SELECT_BY_POS,MODE_TRADES))
		{
			...
			{			
				...
				SymPos = ((int)OrderMagicNumber()) /100000;
				NowMagicNumber = OrderMagicNumber() - SymPos *100000;
...
void monitoraccountprofit()
{
	...
	int localday,difftime;

	/*原则上采用服务器交易时间，为了便于人性化处理，做了一个转换*/	
	timelocal = TimeCurrent() + globaltimezonediff*60*60;


	localday = TimeDay(timelocal);
	difftime = localday -gstarthour ;
	if(difftime <0)
	{
		difftime = difftime +31;
	}
	...
	//if(1 == Period())
	{
		...
		if ((difftime >= 0 )&& (difftime <=9 )) 
		{
			...
			{
				Print("1.1 This turn Own more than "+(symbolNum/2)+" orders witch is "+ordercountall()+" all profit order,Close all");	
		...
		else if ((difftime > 9 )&& (difftime <= 19 )) 
		{
			...
			{
				Print("1.2 This turn Own more than  "+(symbolNum/3)+"  orders witch is "+ordercountall()+" all profit order,Close all");					
		...
		else if ((difftime > 19 )&& (difftime <= 31 ))
		{
			...
			{
				Print("1.3 This turn Own more than  "+(symbolNum/4)+"  orders witch is "+ordercountall()+" all profit order,Close all");					
		...
		else
		{
			...
			if((ordercountall()>=(symbolNum/5))&&(ordercountall() == profitordercountall(0)))
			{
				Print("1.4 This turn Own more than  "+(symbolNum/5)+"  orders witch is "+ordercountall()+" all profit order,Close all");					
		...
		if((ordercountwithprofit(400*mylots0)>= 5)&&(orderprofitall()>0))
		{
		...
		if((ordercountwithprofit(500*mylots0)>= 4)&&(orderprofitall()>0))
		{
		...
		if((ordercountwithprofit(700*mylots0)>= 3)&&(orderprofitall()>0))
		{		
		...
		if((ordercountwithprofit(1000*mylots0)>= 2)&&(orderprofitall()>0))
		{
...
/*三十分钟寻找介入点，对于错误的介入点，通过止损的方式退出，以及人工审视，因此不再考虑对冲机制*/
/*考虑到通常情况下30分钟的点是比较稳定的点，因此充分将止损点设置比较小*/
void orderbuyselltypeone(int SymPos)
{
			...
			)		
	{
		...
		orderStopless = boll_low_B-bool_length*4;		
		/*
	...
	if((BoolCrossRecord[SymPos][timeperiodnum+1].StrongWeak<0.8)
		...
		/*买入持仓单隔夜利息为正*/
		&&(MarketInfo(my_symbol,MODE_SWAPLONG)>0)		
...
////////////////////////////////////////////////////////////////////////
		...
		)
	{
		...
		orderStopless = boll_up_B + bool_length*4;
	...
	if((BoolCrossRecord[SymPos][timeperiodnum+1].StrongWeak>0.2)
		...
		/*卖出持仓单隔夜利息为正*/	
		&&(MarketInfo(my_symbol,MODE_SWAPSHORT)>0)	
...
/*H4寻找介入点，对于错误的介入点，通过止损的方式退出，以及人工审视，因此不再考虑对冲机制*/
/*考虑到通常情况下H4的点是比较稳定的点，因此充分将止损点设置比较小*/
