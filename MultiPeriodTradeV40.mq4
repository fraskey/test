	...
	for (i = 1; i< 500;i++)
	{
      ...
      myboll_mid_B_pre = (myboll_up_B_pre + myboll_low_B_pre)/2;
		...
		if(0 != 	crossflag)		
		{
				...
				BoolCrossRecord[j].CrossBoolPos = iBars(NULL,0)-i;
				j++;
				if (j >= 9)
				{
					break;
...
int init()
{
      ...
      else if (5 == Period() )
      {
          ...
          if(GlobalVariableCheck("g_FiveM_BoolTime") == TRUE)
			  ...
			  if(GlobalVariableCheck("g_FiveMCrossPos") == TRUE)
		      {      
	          Print("g_FiveMCrossPos already exist" );        
		      }
		      else
		      {

		      		GlobalVariableSet("g_FiveMCrossPos",0);
		      	  if(GlobalVariableCheck("g_FiveMCrossPos") == FALSE)
		      	  {
		          	Print("init False due to g_FiveMCrossPos set false!");  
		          	return -1;     		      	  
		      	  }		    
		      	  else
		      	  {
          			Print("init g_FiveMCrossPos is OK  ");  		          			      	  
		      	  }  
   ...
   for (i = 0; i < 100; i++)
   {
		...
		||(GlobalVariableCheck("g_FiveMCrossPos") == FALSE)||(GlobalVariableCheck("g_FiveM_Direction") == FALSE))
	   	{
	...
	//删除自己的全局变量
      ...
      else if (5 == Period() )
      {
		...
		{      
		  ...
		  if(GlobalVariableCheck("g_FiveMCrossPos") == TRUE)
		{      
		  GlobalVariableDel("g_FiveMCrossPos");
...
void ChangeCrossValue( int mvalue)
{
	...
	for (i = 0 ; i <9; i++)
	{
		...
		BoolCrossRecord[9-i].CrossBoolPos = BoolCrossRecord[8-i].CrossBoolPos ;
	}
	
	BoolCrossRecord[0].CrossFlag = mvalue;
	BoolCrossRecord[0].CrossDatetime = TimeCurrent();
	BoolCrossRecord[0].CrossBoolPos = iBars(NULL,0);
	return;
}
...
bool OneMKeepBalance(datetime mydatetime,int balancetime)
{
...
bool OneMStrongTrendChangeDown(datetime mydatetime,int balancetime)
{
...
bool OneMStrongTrendChangeUp(datetime mydatetime,int balancetime)
{
...
//---
   ...
   if (1 == Period() )
   {
		...
		else
		{
			...
			if((Ask>(FiveM_BoolMidLine+FiveM_BoolDistance))&&((TimeCurrent()-OneMInBoolRecTime)<360)
			...
			if((Bid<(FiveM_BoolMidLine-FiveM_BoolDistance))&&((TimeCurrent()-OneMInBoolRecTime)<360)
...
//////////////////////////////////////////////////////////////////////////////
  ...
  {		
   ...
   /*获取必要参数*/
		...
		GlobalVariableSet("g_FiveM_Direction",BoolCrossRecord[0].CrossFlag);
		GlobalVariableSet("g_FiveM_BoolDistance",bool_length);			
		GlobalVariableSet("g_FiveM_BoolMidLine",boll_mid_B);

		if((5==crossflag)||(-5==crossflag)||(-4==crossflag)||(4==crossflag))
		{
		 GlobalVariableSet("g_FiveM_BoolTime",BoolCrossRecord[0].CrossDatetime); 
		 GlobalVariableSet("g_FiveMCrossPos",BoolCrossRecord[0].CrossBoolPos);
		}
		...
		&&(BoolCrossRecord[2].CrossFlag==-1)&&(Ask <(boll_mid_B-bool_length*0.4))) )
		{
			...
			GlobalVariableSet("g_FiveMCrossPos",1);
		}
	...
	/*获取必要参数*/
	 ...
	 ThirtyM_Direction = GlobalVariableGet("g_ThirtyM_Direction");
	   ...
	   //寻找最大值，作为本周期做空的止损参考值  
	   if ((ThirtyM_Direction > 4.5)&&(ThirtyM_Direction < 5.5))
	   {
		...
		else
		{
			...
			//寻找最大值，作为本周期做空的止损参考值  
	   if ((ThirtyM_Direction > 3.5) &&(ThirtyM_Direction <4.5))
	   {
	   ...
	   //寻找最小值，作为本周期做多的止损参考值  
	   if ((ThirtyM_Direction > -5.5) &&(ThirtyM_Direction < -4.5))
	   {
	   ...
	   //寻找最小值，作为本周期做多的止损参考值  
	   if ((ThirtyM_Direction > -4.5) &&(ThirtyM_Direction < -3.5))
	   {
...
//////////////////////////////////////////////////////////////
		...
		FiveMCrossPos = GlobalVariableGet("g_FiveMCrossPos");  
		FiveM_BoolTime = GlobalVariableGet("g_FiveM_BoolTime");		
		FiveM_Direction = GlobalVariableGet("g_FiveM_Direction");
			
	   if ((FiveM_Direction > 4.5)&&(FiveM_Direction < 5.5))
	   {
			if(((crossflag == 5 )||(crossflag == 1 ))&&(OneMOrderCloseStatus(MagicNumberSeven)==true)
				&&(OneMKeepBalance(FiveM_BoolTime,1)==true)&&(OneMKeepBalance(FiveM_BoolTime,3)==false))
			{
					MinValue3 = 100000;
					for (i= 0;i < (TimeCurrent()-FiveM_BoolTime)/60;i++)
					{
						if(MinValue3 > iLow(NULL,0,i))
						{
							MinValue3 = iLow(NULL,0,i);
						}
					...
					orderStopLevel =MarketInfo(Symbol(),MODE_STOPLEVEL)*2;
					if(orderStopLevel < 1)
					{
						orderStopLevel = 100;
					}					
	   ...
	   if ((FiveM_Direction > -5.5)&&(FiveM_Direction < -4.5))
	   {

			if(((crossflag == -1 )||(crossflag == -5 ))&&(OneMOrderCloseStatus(MagicNumberEight)==true)
			&&(OneMKeepBalance(FiveM_BoolTime,1)==true)&&(OneMKeepBalance(FiveM_BoolTime,3)==false))
			{
				...
				MaxValue4 = -1;
					for (i= 0;i < (TimeCurrent()-FiveM_BoolTime)/60;i++)
				{
					if(MaxValue4 < iHigh(NULL,0,i))
					{
						MaxValue4 = iHigh(NULL,0,i);
					}					
				}				
		...
		/*5分钟第一次突破下轨，1分钟寻求背驰点买入*/				
		if ((FiveM_Direction <-3.5)&&(FiveM_Direction >-4.5))
		{	  

			if(((crossflag == -4 ))&&(OneMOrderCloseStatus(MagicNumberFive)==true)
				&&(OneMStrongTrendChangeUp(FiveM_BoolTime,3)==true))
			{
					MinValue3 = 100000;
					for (i= 0;i < (TimeCurrent()-FiveM_BoolTime)/60;i++)
					{
		...
		if ((FiveM_Direction >3.5)&&(FiveM_Direction <4.5))
		{	  
			if(((crossflag == 4 ))&&(OneMOrderCloseStatus(MagicNumberSix)==true)
			&&(OneMStrongTrendChangeDown(FiveM_BoolTime,3)==true))
			{
