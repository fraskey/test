		  ...
		  if((NowSymPos<0)||(NowSymPos>=symbolNum))
		  {
			...
			if(NowMagicNumber == MagicNumberOne)
			{
				...
				{   
				   ...
				   else
				   {
					...
					else if(ThirtyM_StrongWeak<0.8)
					{
						...
						if((vclose <BuySellPosRecord[NowSymPos].RecentMin[0])&&(vclose <boll_low_B))
						{
							 vbid    = MarketInfo(MySymbol[NowSymPos],MODE_BID);
							 ticket =OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);					  
							 if(ticket <0)
			...
			if(NowMagicNumber == MagicNumberTwo)
			{
				...
				//////////////////////////////////////////   
					...
					else if(ThirtyM_StrongWeak>0.2)
					{
						...
						if((vclose > BuySellPosRecord[NowSymPos].RecentMax[1])&&(vclose > boll_up_B))
						{
							vask    = MarketInfo(MySymbol[NowSymPos],MODE_ASK);
							 ticket = OrderClose(OrderTicket(),OrderLots(),vask,3,Red);
							 if(ticket <0)
...
/////////////////////////////////////////
					...
					else if(FiveM_StrongWeak<0.8)
					{
						...
						vbid    = MarketInfo(MySymbol[NowSymPos],MODE_BID);
						vclose  = iClose(g_symbol,0,1);
						if((vclose <BuySellPosRecord[NowSymPos].RecentMin[2])&&(vclose <boll_low_B))
						{
							 vbid    = MarketInfo(MySymbol[NowSymPos],MODE_BID);
							 ticket =OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);					  
							 if(ticket <0)
				...
				{   	   
				   ...
				   else
				   {
					...
					else if(FiveM_StrongWeak>0.2)
					{
						...
						vask    = MarketInfo(g_symbol,MODE_ASK);
						vclose  = iClose(g_symbol,0,1);
						if((vclose > BuySellPosRecord[NowSymPos].RecentMax[3])&&(vclose > boll_up_B))
						{
							vask    = MarketInfo(MySymbol[NowSymPos],MODE_ASK);
							 ticket = OrderClose(OrderTicket(),OrderLots(),vask,3,Red);
							 if(ticket <0)
