...
void initglobalamount()
{
	...
	else if((AccountFreeMargin() > 200)&&(AccountFreeMargin() <= 300))
	{
		MyLotsH          =0.02;
		MyLotsL          =0.02;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}
	else if((AccountFreeMargin() > 300)&&(AccountFreeMargin() <= 400))
	{
		MyLotsH          =0.03;
		MyLotsL          =0.03;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}
	else if((AccountFreeMargin() > 400)&&(AccountFreeMargin() <= 500))
	{
		MyLotsH          =0.04;
		MyLotsL          =0.04;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}
	else if((AccountFreeMargin() > 500)&&(AccountFreeMargin() <= 600))
	{
		MyLotsH          =0.05;
		MyLotsL          =0.05;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}	
	else if((AccountFreeMargin() > 600)&&(AccountFreeMargin() <= 700))
	{
		MyLotsH          =0.06;
		MyLotsL          =0.06;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}
	else if((AccountFreeMargin() > 700)&&(AccountFreeMargin() <= 800))
	{
		MyLotsH          =0.07;
		MyLotsL          =0.07;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}
	else if((AccountFreeMargin() > 800)&&(AccountFreeMargin() <= 900))
	{
		MyLotsH          =0.08;
		MyLotsL          =0.08;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}
	else if((AccountFreeMargin() > 900)&&(AccountFreeMargin() <= 1000))
	{
		MyLotsH          =0.09;
		MyLotsL          =0.09;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}		
	else if((AccountFreeMargin() > 1000)&&(AccountFreeMargin() <= 4000))
	{
		MyLotsH          =0.1;
		MyLotsL          =0.1;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}		
	else if((AccountFreeMargin() > 4000)&&(AccountFreeMargin() <= 8000))
	{
		MyLotsH          =0.2;
		MyLotsL          =0.2;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	...
	else if((AccountFreeMargin() > 8000)&&(AccountFreeMargin() <= 10000))
	{
		MyLotsH          =0.3;
		MyLotsL          =0.3;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}			
	else if((AccountFreeMargin() > 10000)&&(AccountFreeMargin() <= 50000))
	{
		MyLotsH          =0.5;
		MyLotsL          =0.5;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}		
	else if((AccountFreeMargin() > 50000)&&(AccountFreeMargin() <= 100000))
	{
		MyLotsH          =1;
		MyLotsL          =1;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	...
	{	
		...
		{					
			...
			if(AccountFreeMargin() <= 200)
			{
				MyLotsH          =0.01;
				MyLotsL          =0.01;	
				Print("Auto adjest Amount is = "+MyLotsH+":"+MyLotsL);	 				
			}
			else if((AccountFreeMargin() > 200)&&(AccountFreeMargin() <= 300))
			{
				MyLotsH          =0.02;
			...
			else
			{
				MyLotsH          =0.01;
				MyLotsL          =0.01;	
				Print("default Auto adjest Amount is = "+MyLotsH+":"+MyLotsL);	 							
			}		
		}		
...
//	int i;
  ...
  symbol = MySymbol[SymPos];
	...
	if ((TimeHour(timelocal) >= 14 )&& (TimeHour(timelocal) <20 )) 
	{
		tradetimeflag = true;		
	}	
	/*测试期间全时间段交易*/
...
//	int i;
  ...
  symbol = MySymbol[SymPos];
	...
	if ((TimeHour(timelocal) >= 16 )&& (TimeHour(timelocal) <19 )) 
	{
		tradetimeflag = true;		
	}	
	/*测试期间全时间段交易*/
...
bool isvalidmagicnumber(int magicnumber)
{
	...
	if(0!= (NowMagicNumber%10))
	{
	 flag = false;
	}	
	
	NowMagicNumber = ((int)NowMagicNumber) /10;
	if((NowMagicNumber<=0)||(NowMagicNumber>=17))
	{
	 flag = false;
	}	
...
/*默认晚上23点后强制关闭所有订单，实际情况是如果晚上8:30以后有重要数据，建议8:10之前平仓所有订单，不要受重大数据影响*/
/*坚决不参与重大数据发布行情*/
void forcecloseall(int hour,int minute)
{
  ...
  timelocal = TimeCurrent() + globaltimezonediff*60*60;
	...
	/*确保交易时间段，来临前开启全局交易交易标记*/
	if ((TimeHour(timelocal) == hour )&& (TimeMinute(timelocal) >=  minute)) 
	{			
		//确保是每个周期五分钟计算一次，而不是每个tick计算一次
		if ( BoolCrossRecord[SymPos][timeperiodnum].ChartEvent != iBars(my_symbol,my_timeperiod))
		{		
...
void monitoraccountprofit()
{
	...
	//if(1 == Period())
	{
		...
		if ((TimeHour(timelocal) >= 14 )&& (TimeHour(timelocal) <=19 )) 
		{
					
			/*超过9个订单，且每个订单都盈利的情况下，直接关掉所有盈利订单*/
			
			if((ordercountall()>=(symbolNum/4))&&(ordercountall() == profitordercountall(0)))
			{
				...
				Sleep(1000); 
				ordercloseallwithprofit(-100);				
				Print("1 This turn Own more than "+(symbolNum/4)+" Eight orders witch is "+ordercountall()+" all profit order,Close all");			
		...
		else if ((TimeHour(timelocal) > 19 )&& (TimeHour(timelocal) <= 20 )) 
		{
			
			/*超过8个订单，且每个订单都盈利的情况下，直接关掉所有盈利订单*/
			
			if((ordercountall()>=(symbolNum/8))&&(ordercountall() == profitordercountall(0)))
			{
				...
				Sleep(1000); 
				ordercloseallwithprofit(-100);
				Print("1 This turn Own more than  "+(symbolNum/8)+"  orders witch is "+ordercountall()+" all profit order,Close all");			
				
			}		
			mylots0 = MyLotsH*0.5;	
			mylots = MyLotsH*0.75*0.75;					
		...
		else
		{
			...
			if((ordercountall()>=(symbolNum/12))&&(ordercountall() == profitordercountall(0)))
			{
				...
				Sleep(1000); 
				ordercloseallwithprofit(-100);
				Print("1 This turn Own more than  "+(symbolNum/12)+"  orders witch is "+ordercountall()+" all profit order,Close all");			
				
			}		
			mylots0 = MyLotsH*0.5*0.75;	
			mylots = MyLotsH*0.75*0.75*0.75;					
			
		}				

		
		/*盈利单的盈利总和超过1000美金，直接关掉所有盈利订单，落袋为安，完成一次循环*/
		if(profitorderprofitall() > 10000*mylots0)
		{	
				...
				Sleep(1000); 
				ordercloseallwithprofit(-100);			
			Print("2 This turn Own more than "+5000*mylots0+" USD,Close all");
		}
		...
		/*四个以上40美金订单，直接关掉所有盈利订单，落袋为安，完成一次循环*/
		if((ordercountwithprofit(400*mylots0)>= 4)&&(orderprofitall()>0))
		{
			/*一波做完后，手工禁止交易；第二天继续做*/
			setglobaltradeflag(false);
			subject = g_forexserver +":All Orders Closed Now,Please Close Other Oders quickly";
			...
			Sleep(1000); 
			ordercloseallwithprofit(-100);			
			Print("4 This turn Own more than Four "+400*mylots0+" USD,Close all");
		}

		/*三个以上50美金订单，直接关掉所有盈利订单，落袋为安，完成一次循环*/
		if((ordercountwithprofit(500*mylots0)>= 3)&&(orderprofitall()>0))
		{
			...
			Sleep(1000); 
			ordercloseallwithprofit(-100);			
			Print("5 This turn Own more than Three "+500*mylots0+" USD,Close all");
		}

		/*两个以上70美金订单，直接关掉所有盈利订单，落袋为安，完成一次循环*/
		if((ordercountwithprofit(700*mylots0)>= 2)&&(orderprofitall()>0))
		{		
			...
			Sleep(1000); 
			ordercloseallwithprofit(-100);			
			Print("6 This turn Own more than two "+700*mylots0+" USD,Close all");
		}
		
		/*一个以上100美金订单，直接关掉所有盈利订单，落袋为安，完成一次循环*/
		if((ordercountwithprofit(1000*mylots0)>= 1)&&(orderprofitall()>0))
		{
			/*一波做完后，手工禁止交易；第二天继续做*/
			setglobaltradeflag(false);
			subject = g_forexserver +":All Orders Closed Now,Please Close Other Oders quickly";
			...
			Sleep(1000); 
			ordercloseallwithprofit(-100);			
			Print("7 1、This turn Own more than one "+1000*mylots0+" USD,Close all");
		}
...
void orderbuyselltypetwo(int SymPos)
{
	...
	//小周期突破，大周期回调结束
	//属于中周期深度调整型
	//做多的时间控制在下午4点到晚上7点之间，这个时间段通常能形成趋势，对冲高点买入的风险
	//小周期突破的中周期深度回调型买点
