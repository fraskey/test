...
void initglobalamount()
{
  ...
  timelocal = TimeCurrent() + globaltimezonediff*60*60;
	...
	//周一1/4手数下单，原因是周一通常是变盘时间节点，震荡幅度比较大
	if (TimeDayOfWeek(timelocal) == 1)
	{
		...
		Print("!we change default Amount to 1/4 because of wensday which is = "+MyLotsH+":"+MyLotsL);	 			
  ...
  timelocal = TimeCurrent() + globaltimezonediff*60*60;
	...
	{	
		...
		{					
			...
			//周一1/3手数下单，原因是周一通常是变盘时间节点，震荡幅度比较大
			if (TimeDayOfWeek(timelocal) == 1)
			{
				MyLotsH          =MyLotsH/4;
				MyLotsL          =MyLotsL/4;	
				...
				MyLotsH = NormalizeDouble(MyLotsH,2);		
				MyLotsL = NormalizeDouble(MyLotsL,2);		
				Print("!!!Auto adjest we change default Amount to 1/4 because of wensday which is = "+MyLotsH+":"+MyLotsL);	 			
...
/*默认晚上0:30点后强制关闭所有订单，实际情况是如果晚上8:30以后有重要数据，建议8:10之前平仓所有订单，不要受重大数据影响*/
/*坚决不参与重大数据发布行情*/
...
void monitoraccountprofit()
{
	...
	//if(1 == Period())
	{
		...
		else if (((TimeHour(timelocal) > 0 )&& (TimeHour(timelocal) <= 3 )) ||(TimeHour(timelocal) == 0 ))
		{
		...
		else
		{
			...
			{
				Print("1.4 This turn Own more than  "+(symbolNum/12)+"  orders witch is "+ordercountall()+" all profit order,Close all");					
				/*一波做完后，手工禁止交易；第二天继续做*/
				setglobaltradeflag(false);
				subject = g_forexserver +":All Orders Closed Now,Please Close Other Oders quickly";
				SendMail( subject,some_text);		
...
void orderbuyselltypeone(int SymPos)
{
			...
			)		
	{
		...
		*/
		orderTakeProfit	= 	orderPrice + bool_length*24;
		/*参数修正*/ 
			...
			)		
	{
		...
		orderPrice = vask;				 
		//orderStopless =MinValue3- bool_length*4; 	
		orderStopless = boll_low_B-bool_length*10;		
		/*
		if((orderPrice - orderStopless)>bool_length*2)
		{
			orderStopless = orderPrice - bool_length*2;
...
////////////////////////////////////////////////////////////////////////
			...
			)
	{
		...
		orderPrice = vbid;						 
		//orderStopless =MaxValue4 + bool_length*4; 
		orderStopless = boll_up_B + bool_length*10;
