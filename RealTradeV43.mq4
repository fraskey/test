...
void initglobalamount()
{
  ...
  timelocal = TimeCurrent() + globaltimezonediff*60*60;
	...
	else if((AccountBalance() > 200)&&(AccountBalance() <= 400))
	{
		MyLotsH          =0.02;
		MyLotsL          =0.02;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}
	else if((AccountBalance() > 400)&&(AccountBalance() <= 600))
	{
		MyLotsH          =0.03;
		MyLotsL          =0.03;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}
	else if((AccountBalance() > 600)&&(AccountBalance() <= 800))
	{
		MyLotsH          =0.04;
		MyLotsL          =0.04;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}
	else if((AccountBalance() > 800)&&(AccountBalance() <= 1000))
	{
		MyLotsH          =0.05;
		MyLotsL          =0.05;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}	
	else if((AccountBalance() > 1000)&&(AccountBalance() <= 1400))
	{
		MyLotsH          =0.06;
		MyLotsL          =0.06;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}
	else if((AccountBalance() > 1400)&&(AccountBalance() <= 1800))
	{
		MyLotsH          =0.07;
		MyLotsL          =0.07;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}
	else if((AccountBalance() > 1800)&&(AccountBalance() <= 2200))
	{
		MyLotsH          =0.08;
		MyLotsL          =0.08;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}
	else if((AccountBalance() > 2200)&&(AccountBalance() <= 2600))
	{
		MyLotsH          =0.09;
		MyLotsL          =0.09;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}		
	else if((AccountBalance() > 2600)&&(AccountBalance() <= 3000))
	{
		MyLotsH          =0.1;
		MyLotsL          =0.1;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	...
	else if((AccountBalance() > 5000)&&(AccountBalance() <= 6000))
	{
		MyLotsH          =0.16;
		MyLotsL          =0.16;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}		
	else if((AccountBalance() > 6000)&&(AccountBalance() <= 7000))
	{
		MyLotsH          =0.18;
		MyLotsL          =0.18;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}	
	else if((AccountBalance() > 7000)&&(AccountBalance() <= 8000))
	{
		MyLotsH          =0.20;
		MyLotsL          =0.20;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	...
	else if((AccountBalance() > 40000)&&(AccountBalance() <= 80000))
	{
		MyLotsH          =1;
		MyLotsL          =1;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}			
	else if((AccountBalance() > 80000)&&(AccountBalance() <= 200000))
	{
		MyLotsH          =2;
		MyLotsL          =2;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	...
	if(AccountServer() == HLMAXSERVER)
	{
		...
		Print("!LMax test default init Amount is = "+MyLotsH+":"+MyLotsL);	 		
	}				
	...
	{	
		...
		{					
				...
				if(AccountServer() == HLMAXSERVER)
				{
					...
					Print("!LMax test default Auto adjest Amount is = "+MyLotsH+":"+MyLotsL);	 		
				}				
...
//	else
	{
   	...
   	if ((TimeHour(timelocal) >= 13 )&& (TimeHour(timelocal) <20 )) 
   	{
		...
		/*五个以上40美金订单，直接关掉所有盈利订单，落袋为安，完成一次循环*/
		if((anti_ordercountwithprofit(400*mylots0)>= 5)&&(anti_orderprofitall()>0))
		{
			anti_turnoffflag = true;						
			Print("4 This turn Own more than Five "+400*mylots0+" USD,Close all");
		}
		...
		/*四个以上50美金订单，直接关掉所有盈利订单，落袋为安，完成一次循环*/
		if((anti_ordercountwithprofit(500*mylots0)>= 4)&&(anti_orderprofitall()>0))
		{
		...
		/*三个以上70美金订单，直接关掉所有盈利订单，落袋为安，完成一次循环*/
		if((anti_ordercountwithprofit(700*mylots0)>= 3)&&(anti_orderprofitall()>0))
		{		
...
void all_monitoraccountprofit()
{
	...
	//if(1 == Period())
	{
		...
		*/
		...
		if((all_ordercountwithprofit(300*mylots0)>= 8)&&(all_orderprofitall()>0))
		{
			turnoffflag = true;					
			Print("4 This turn Own more than eight "+300*mylots0+" USD,Close all");
		}

		/*六个以上40美金订单，直接关掉所有盈利订单，落袋为安，完成一次循环*/
		if((all_ordercountwithprofit(400*mylots0)>= 6)&&(all_orderprofitall()>0))
		{
			turnoffflag = true;					
			Print("4 This turn Own more than six "+400*mylots0+" USD,Close all");
		}

		/*五个以上50美金订单，直接关掉所有盈利订单，落袋为安，完成一次循环*/
		if((all_ordercountwithprofit(500*mylots0)>= 5)&&(all_orderprofitall()>0))
		{
			
			turnoffflag = true;				
			Print("5 This turn Own more than Five "+500*mylots0+" USD,Close all");
		}

		/*四个以上70美金订单，直接关掉所有盈利订单，落袋为安，完成一次循环*/
		if((all_ordercountwithprofit(700*mylots0)>= 4)&&(all_orderprofitall()>0))
		{		
			
			turnoffflag = true;				
			Print("6 This turn Own more than four "+700*mylots0+" USD,Close all");
		}
		
		/*三个以上100美金订单，直接关掉所有盈利订单，落袋为安，完成一次循环*/
		if((all_ordercountwithprofit(1000*mylots0)>= 3)&&(all_orderprofitall()>0))
		{
			turnoffflag = true;			
			Print("7 1、This turn Own more than three "+1000*mylots0+" USD,Close all");
		}
		
		/*两个以上150美金订单，直接关掉所有盈利订单，落袋为安，完成一次循环*/
		if((all_ordercountwithprofit(1500*mylots0)>= 2)&&(all_orderprofitall()>0))
		{
			turnoffflag = true;			
			Print("7 2、This turn Own more than two "+1500*mylots0+" USD,Close all");
		}
