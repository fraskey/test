...
void transferfiletoflag()
{
	...
	for(i = 0; i < tradefileNum;i++)
	{
		...
		MyAntiTradeFlag[i] = ReadFileInt(MyAntiTradeFlag[i]);		
...
void initglobalamount()
{
  ...
  timelocal = TimeCurrent() + globaltimezonediff*60*60;
	...
	else if((AccountBalance() > 3000)&&(AccountBalance() <= 4500))
	{
		MyLotsH          =0.06;
		MyLotsL          =0.03;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}
	else if((AccountBalance() > 4500)&&(AccountBalance() <= 6000))
	{
		MyLotsH          =0.08;
		MyLotsL          =0.04;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}
	else if((AccountBalance() > 6000)&&(AccountBalance() <= 7500))
	{
		MyLotsH          =0.10;
		MyLotsL          =0.05;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}	
	else if((AccountBalance() > 7500)&&(AccountBalance() <= 10000))
	{
		MyLotsH          =0.12;
		MyLotsL          =0.06;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}
	else if((AccountBalance() > 10000)&&(AccountBalance() <= 15000))
	{
		MyLotsH          =0.16;
		MyLotsL          =0.08;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
	}
	else if((AccountBalance() > 15000)&&(AccountBalance() <= 20000))
	{
		MyLotsH          =0.20;
		MyLotsL          =0.10;	
		Print("init Amount is = "+MyLotsH+":"+MyLotsL);	 							
...
void monitoraccountprofit()
{
	...
	if((true ==tradedok())&&(orderprofitall() >0))
	{
			turnoffflag = true;			
...
void all_monitoraccountprofit()
{
	...
	//当发现别的平台有盈利平仓后，本平台也盈利平仓
	if((true ==alltradedok())&&(all_orderprofitall() >0))
	{
			turnoffflag = true;			
