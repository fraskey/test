...
void InitMA(int SymPos)
{
	...
	double StrongWeak;
	string m_symbol;
	m_symbol = MySymbol[SymPos];
	
	
	MAThree=iMA(m_symbol,0,3,0,MODE_SMA,PRICE_CLOSE,0); 
	MAThen=iMA(m_symbol,0,10,0,MODE_SMA,PRICE_CLOSE,0);  
	MAThenPre=iMA(m_symbol,0,10,0,MODE_SMA,PRICE_CLOSE,1); 

	
	MAFive=iMA(m_symbol,0,5,0,MODE_SMA,PRICE_CLOSE,0); 
	MAThentyOne=iMA(m_symbol,0,21,0,MODE_SMA,PRICE_CLOSE,0); 
	MASixty=iMA(m_symbol,0,60,0,MODE_SMA,PRICE_CLOSE,0); 
 
	MAFivePre=iMA(m_symbol,0,5,0,MODE_SMA,PRICE_CLOSE,1); 
	MAThentyOnePre=iMA(m_symbol,0,21,0,MODE_SMA,PRICE_CLOSE,1); 
	MASixtyPre=iMA(m_symbol,0,60,0,MODE_SMA,PRICE_CLOSE,1); 
	...
	if (240 == Period() )
	{
		GlobalVariableSet("g_FourH_Trend"+m_symbol,StrongWeak);   	 
	}
	else if (1440 == Period() )
	{
		GlobalVariableSet("g_OneD_Trend"+m_symbol,StrongWeak);   	 
	}
	else
	{
		;   
  ...
  if (240 == Period() )
  {
	GlobalVariableSet("g_FourH_SW"+m_symbol,StrongWeak);   
	 
  }
  else if (30 == Period() )
  {
	GlobalVariableSet("g_ThirtyM_SW"+m_symbol,StrongWeak);   
	 
  }
   else if (5 == Period() )
  {
	GlobalVariableSet("g_FiveM_SW"+m_symbol,StrongWeak);   
...
/*交易时间段多以趋势和趋势加强为主，非交易时间多以震荡为主，以此区分一些小周期的交易策略*/
...
//	int i;
	...
	if ((TimeHour(TimeLocal()) < 15 )&& (TimeHour(TimeLocal()) >1 )) //下午3点前不做趋势单，主要针对1分钟线，非欧美时间趋势不明显
	{
		tradetimeflag = false;		
	}	
	return tradetimeflag;
   ...
   for(SymPos = 0; SymPos < symbolNum;SymPos++)
   {
      ...
      /*获取必要参数*/
		...
		ThirtyM_StrongWeak = GlobalVariableGet("g_ThirtyM_SW"+my_symbol);
   		...
   		if((FourH_StrongWeak>0.8)&&(OneD_Trend>0.8)
						...
						{				
							if(((int)OrderMagicNumber()) == MakeMagic(SymPos,MagicNumberOne))
							{
								...
								if(orderStopless >OrderStopLoss() )
								{
									...
									Print(my_symbol+" MagicNumberOne2 Modify:" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
													+"MagicNumberOnePos="+BuySellPosRecord[SymPos].MagicNumberOnePos);										
									res=OrderModify(OrderTicket(),OrderOpenPrice(),
										   orderStopless,orderTakeProfit,0,clrPurple);
   		...
   		//大周期处于空头市场，本周期在上涨背驰阶段买入，趋势交易，目的是为了找到比较好的入场点，和止损点
   		if((FourH_StrongWeak<0.2)&&(OneD_Trend<0.2)
						...
						{				
							if(((int)OrderMagicNumber()) == MakeMagic(SymPos,MagicNumberTwo))
							{
								...
								if(orderStopless < OrderStopLoss() )
								{
									...
									Print(my_symbol+" MagicNumberTwo2 Modify:" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
													+"MagicNumberTwoPos="+BuySellPosRecord[SymPos].MagicNumberTwoPos);	
      ...
      /*获取必要参数*/
		FourH_Trend = GlobalVariableGet("g_FourH_Trend"+my_symbol);      
   		...
   		//大周期处于多头市场，本周期在下跌背驰阶段买入，趋势交易，目的是为了找到比较好的入场点，和止损点
   		if((ThirtyM_StrongWeak>0.8)&&(FourH_Trend>0.8)
			...
			&&((BoolCrossRecord[SymPos].CrossBoolPos[5]-BuySellPosRecord[SymPos].MagicNumberThreePos)>0))			
   			{
				...
				BuySellPosRecord[SymPos].MagicNumberThreePos	= BoolCrossRecord[SymPos].CrossBoolPos[0];	
				Print(my_symbol+" MagicNumberThree2 OrderSend/Modify:" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
								+"MagicNumberThreePos="+BuySellPosRecord[SymPos].MagicNumberThreePos);				
				if(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberThree))==true)
				{
															 
					ticket = OrderSend(my_symbol,OP_BUY,orderLots,orderPrice,3,orderStopless,0,
								   my_symbol+"MagicNumberThree2",MakeMagic(SymPos,MagicNumberThree),0,Blue);
   		...
   		if((ThirtyM_StrongWeak>0.8)&&(FourH_Trend>0.8)
			&&(FiveM_StrongWeak<0.8)&&(FiveM_StrongWeak>0.2)
			...
			&&(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberThree))==true))				
   			{
				...
				if(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberOne))==false)
				{
					...
					for (j = 0; j < OrdersTotal(); j++)
					{
						...
						{				
							if(((int)OrderMagicNumber()) == MakeMagic(SymPos,MagicNumberOne))
							{
								...
								else if(tradetimecheck(SymPos) ==false)
								{
				...
				else if(tradetimecheck(SymPos) == false)
				{
					...
					*/
					...
					Print(my_symbol+" MagicNumberThree5 OrderSend:" + "orderLots=" + orderLots +"orderPrice ="
								+orderPrice+"orderStopless="+orderStopless
								+"MagicNumberThreePos="+BuySellPosRecord[SymPos].MagicNumberThreePos);	
										
					ticket = OrderSend(my_symbol,OP_BUY,orderLots,orderPrice,3,orderStopless,orderTakeProfit,
								   my_symbol+"MagicNumberThree5",MakeMagic(SymPos,MagicNumberThree),0,Blue);
		
					 if(ticket <0)
					 {
						Print("OrderSend MagicNumberThree5 failed with error #",GetLastError());
					 }
					 else
					 {            
						BuySellPosRecord[SymPos].TradeTime[2] = TimeCurrent();				 				 
						Print("OrderSend MagicNumberThree5  successfully");
					 }	
   		...
   		//大周期处于空头市场，本周期在上涨背驰阶段买入，趋势交易，目的是为了找到比较好的入场点，和止损点
   		if((ThirtyM_StrongWeak<0.2)&&(FourH_Trend<0.2)
			...
			&&((BoolCrossRecord[SymPos].CrossBoolPos[5]-BuySellPosRecord[SymPos].MagicNumberFourPos)>0))		
   			{
   				...
   				vdigits = (int)MarketInfo(my_symbol,MODE_DIGITS);				
   				MaxValue4 = -1;
   				for (i= 0;i < (iBars(my_symbol,0) -BoolCrossRecord[SymPos].CrossBoolPos[5]);i++)
   				{
   					if(MaxValue4 < iHigh(my_symbol,0,i))
   					{
   						MaxValue4 = iHigh(my_symbol,0,i);
				...
				BuySellPosRecord[SymPos].MagicNumberFourPos	= BoolCrossRecord[SymPos].CrossBoolPos[0];					
				Print(my_symbol+" MagicNumberFour2 OrderSend/Modify:" + "orderLots=" + orderLots +"orderPrice ="
				                +orderPrice+"orderStopless="+orderStopless
								+"MagicNumberFourPos="+BuySellPosRecord[SymPos].MagicNumberFourPos);	
				...
				if(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberFour))==true)
				{
					  ...
					  ticket = OrderSend(my_symbol,OP_SELL,orderLots,orderPrice,3,orderStopless,orderTakeProfit,
									   my_symbol+"MagicNumberFour2",MakeMagic(SymPos,MagicNumberFour),0,Blue);
			...
			&&(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberFour))==true))
   			{
				...
				if(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberTwo))==false)
				{
					for (j = 0; j < OrdersTotal(); j++)
					{
						if (OrderSelect(j,SELECT_BY_POS,MODE_TRADES))
				...
				else if(tradetimecheck(SymPos) ==false)
				{
					...
					*/
					...
					Print(my_symbol+" MagicNumberFour5 OrderSend" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
										+"MagicNumberFourPos="+BuySellPosRecord[SymPos].MagicNumberFourPos);							
					 ticket = OrderSend(my_symbol,OP_SELL,orderLots,orderPrice,3,orderStopless,orderTakeProfit,
									   my_symbol+"MagicNumberFour5",MakeMagic(SymPos,MagicNumberFour),0,Blue);
			
					 if(ticket <0)
					 {
						Print("OrderSend MagicNumberFour5 failed with error #",GetLastError());
					 }
					 else
					 {   
						BuySellPosRecord[SymPos].TradeTime[3] = TimeCurrent();				 					 
						Print("OrderSend MagicNumberFour5  successfully");
					 }
				...
				{   
					...
					if((ThirtyM_StrongWeak<0.8)||(FourH_StrongWeak<0.8))
					{
						...
						if(OrderTakeProfit() < 0.1)
						{
							...
							orderTakeProfit = BuySellPosRecord[SymPos].TakeProfit[0];
							orderTakeProfit = NormalizeDouble(orderTakeProfit,vdigits);
				   ...
				   else
				   {
					...
					if((FiveM_StrongWeak<0.8)||(ThirtyM_StrongWeak<0.8))
					{
						...
						if(OrderTakeProfit() < 0.1)
						{
							...
							orderTakeProfit = BuySellPosRecord[SymPos].TakeProfit[2];
							orderTakeProfit = NormalizeDouble(orderTakeProfit,vdigits);
				   ...
				   else
				   {
					...
					if((FiveM_StrongWeak>0.2)||(ThirtyM_StrongWeak>0.2))
					{
						...
						if(OrderTakeProfit() < 0.1)
						{
							...
							orderTakeProfit = BuySellPosRecord[SymPos].TakeProfit[3];
							orderTakeProfit = NormalizeDouble(orderTakeProfit,vdigits);
