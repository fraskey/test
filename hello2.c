...
void initbuysellpos(int SymPos)
{
	...
	for (i = 0; i < 10; i++)
	{	
		BuySellPosRecord[SymPos].TradeTime[i] = TimeCurrent();
...
//////////////////////////////////////////////////////////////////////////////
					...
					if(((int)OrderMagicNumber()) == MakeMagic(SymPos,MagicNumberOne))
					{
						...
						if(orderStopless >OrderStopLoss() )
						{
							...
							res=OrderModify(OrderTicket(),OrderOpenPrice(),
							 ...
							 {    
								//BuySellPosRecord[SymPos].TradeTime[0] = TimeCurrent();								 
				...
				*/
				...
				ticket = OrderSend(my_symbol,OP_BUY,orderLots,orderPrice,3,orderStopless,orderTakeProfit,
				 ...
				 {      
					...
					BuySellPosRecord[SymPos].TradeTime[0] = TimeCurrent()+7200*Period();				 				 
					...
					if(((int)OrderMagicNumber()) == MakeMagic(SymPos,MagicNumberTwo))
					{
						...
						if(orderStopless < OrderStopLoss() )
						{
							...
							res=OrderModify(OrderTicket(),OrderOpenPrice(),
							 ...
							 {      
								//BuySellPosRecord[SymPos].TradeTime[1] = TimeCurrent();				 								 
				...
				*/
				...
				Print(my_symbol+" MagicNumberTwo3 OrderSend" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
				 ...
				 {   
					...
					BuySellPosRecord[SymPos].TradeTime[1] = TimeCurrent()+7200*Period();				 					 
				...
				if(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberFive))==true)
				{
					...
					{		
						...
						Print(my_symbol+" MagicNumberFive1 OrderSend:" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
						 ...
						 {            
							BuySellPosRecord[SymPos].TradeTime[4] = TimeCurrent();				 				 
				...
				if(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberSix))==true)
				{
					...
					{						
						...
						ticket = OrderSend(my_symbol,OP_BUY,orderLots,orderPrice,3,orderStopless,orderTakeProfit,
						 ...
						 {            
							BuySellPosRecord[SymPos].TradeTime[5] = TimeCurrent();				 				 
					...
					if(((int)OrderMagicNumber()) == MakeMagic(SymPos,MagicNumberThree))
					{
						...
						if(orderStopless >OrderStopLoss() )
						{
							...
							res=OrderModify(OrderTicket(),OrderOpenPrice(),
							 ...
							 {          
								//BuySellPosRecord[SymPos].TradeTime[2] = TimeCurrent();				 								 
				...
				*/
				...
				ticket = OrderSend(my_symbol,OP_BUY,orderLots,orderPrice,3,orderStopless,orderTakeProfit,
				 ...
				 {     
					...
					BuySellPosRecord[SymPos].TradeTime[2] = TimeCurrent()+7200*Period();				 				 
					...
					if(((int)OrderMagicNumber()) == MakeMagic(SymPos,MagicNumberFour))
					{
						...
						if(orderStopless < OrderStopLoss() )
						{
							...
							res=OrderModify(OrderTicket(),OrderOpenPrice(),
							 ...
							 {       
								//BuySellPosRecord[SymPos].TradeTime[3] = TimeCurrent();				 									 
				...
				*/
				...
				Print(my_symbol+" MagicNumberFour3 OrderSend" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
				 ...
				 {   
					...
					BuySellPosRecord[SymPos].TradeTime[3] = TimeCurrent()+7200*Period();				 					 
			...
			{	  
				...
				if(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberSeven))==true)
				{
					...
					Print(my_symbol+" MagicNumberSeven1 OrderSend:" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
					 ...
					 {          
						BuySellPosRecord[SymPos].TradeTime[6] = TimeCurrent();				 				 
			...
			&&(0.2>BoolCrossRecord[SymPos].CrossFiveM_StrongWeak[6]))			
   			{
				...
				if(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberEight))==true)
				{
					...
					ticket = OrderSend(my_symbol,OP_BUY,orderLots,orderPrice,3,orderStopless,orderTakeProfit,
					 ...
					 {            
				 		BuySellPosRecord[SymPos].TradeTime[7] = TimeCurrent();				 
   ...
   ////////////////////////////////////////////////////////////////////////////////////////////////
       ...
       {	
			...
			if(NowMagicNumber == MagicNumberOne)
			{
				...
				{   
					...
					if((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[0]) > 20)
					{
						/*初始化一个超大值，该值不可能达到*/
					...
					if((ThirtyM_StrongWeak<0.8)||(FourH_StrongWeak<0.8))
					{
					   ...
					   /*在非多头向上的情况下，五分钟120个周期，理论上应该走完了,90周期开始监控时间控制*/
					   if((TimeCurrent()-BuySellPosRecord[SymPos].TradeTime[0])>7200*Period())
					   {
						ticket =OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
					   ...
					   else if((TimeCurrent()-BuySellPosRecord[SymPos].TradeTime[0])>5400*Period())
					   {   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
					...
					if(ThirtyM_StrongWeak<0.2)
					{
						...
						{
							//后退一个时间周期
							BuySellPosRecord[SymPos].TradeTime[0] = BuySellPosRecord[SymPos].TradeTime[0]-
								60*Period();
			...
			if(NowMagicNumber == MagicNumberTwo)
			{
				...
				{   	   
				   ...
				   else
				   {
					...
					if((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[1]) > 20)
					{
						/*初始化一个超大值，该值不可能达到*/
					...
					if((ThirtyM_StrongWeak>0.2)||(FourH_StrongWeak>0.2))
					{
						...
						/*在非多头向下的情况下五分钟120个周期，理论上应该走完了,90分钟开始监控时间控制*/
						if((TimeCurrent()-BuySellPosRecord[SymPos].TradeTime[1])>7200*Period())
						{
						  ticket =OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
						...
						else if((TimeCurrent()-BuySellPosRecord[SymPos].TradeTime[1])>5400*Period())
						{   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
					...
					if(ThirtyM_StrongWeak > 0.8)
					{
						...
						{
							//后退一个时间周期
							BuySellPosRecord[SymPos].TradeTime[1] = BuySellPosRecord[SymPos].TradeTime[1]-
								60*Period();
			...
			if(NowMagicNumber == MagicNumberThree)
			{
				...
				{   
				   ...
				   else
				   {
					...
					if((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[2]) > 20)
					{
						/*初始化一个超大值，该值不可能达到*/
					...
					if((FiveM_StrongWeak<0.8)||(ThirtyM_StrongWeak<0.8))
					{
					   ...
					   if((TimeCurrent()-BuySellPosRecord[SymPos].TradeTime[2])>7200*Period())
					   {
						ticket =OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
					   ...
					   else if((TimeCurrent()-BuySellPosRecord[SymPos].TradeTime[2])>5400*Period())
					   {   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
					...
					if(FiveM_StrongWeak < 0.2)
					{
						...
						{
							//后退一个时间周期
							BuySellPosRecord[SymPos].TradeTime[2] = BuySellPosRecord[SymPos].TradeTime[2]-
								60*Period();
			...
			if(NowMagicNumber == MagicNumberFour)
			{
				...
				{   	   
				   ...
				   else
				   {
					...
					if((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[3]) > 20)
					{
						/*初始化一个超大值，该值不可能达到*/
					...
					if((FiveM_StrongWeak>0.2)||(ThirtyM_StrongWeak>0.2))
					{
						...
						if((TimeCurrent()-BuySellPosRecord[SymPos].TradeTime[3])>7200*Period())
						{
						  ticket =OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
						...
						else if((TimeCurrent()-BuySellPosRecord[SymPos].TradeTime[3])>5400*Period())
						{   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
					...
					if(FiveM_StrongWeak > 0.8)
					{
						...
						{
							//后退一个时间周期
							BuySellPosRecord[SymPos].TradeTime[3] = BuySellPosRecord[SymPos].TradeTime[3]-
								60*Period();
			...
			if(NowMagicNumber == MagicNumberFive)
			{
				...
				{   	   
					...
					if(ThirtyM_StrongWeak>0.2)	
					{
						...
						vask    = MarketInfo(MySymbol[SymPos],MODE_ASK);
					   /*在非多头向下的情况下五分钟120个周期，理论上应该走完了,90分钟开始监控时间控制*/
					   if((TimeCurrent()-BuySellPosRecord[SymPos].TradeTime[4])>7200)
					   {
						  ticket =OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
					   ...
					   else if((TimeCurrent()-BuySellPosRecord[SymPos].TradeTime[4])>5400)
					   {   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
				...
				{   	   
				   ...
				   else
				   {
					...
					if(ThirtyM_StrongWeak<0.8)	
					{
						...
						vbid    = MarketInfo(MySymbol[SymPos],MODE_BID);
					   /*在非多头向上的情况下，五分钟120个周期，理论上应该走完了,90分钟开始监控时间控制*/
					   if((TimeCurrent()-BuySellPosRecord[SymPos].TradeTime[5])>7200)
					   {
						ticket =OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
					   ...
					   else if((TimeCurrent()-BuySellPosRecord[SymPos].TradeTime[5])>5400)
					   {   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
				...
				{   	   
				   ...
				   else
				   {
					...
					if(FiveM_StrongWeak>0.2)	
					{
						...
						vask    = MarketInfo(MySymbol[SymPos],MODE_ASK);
					   /*在非多头向下的情况下一分钟120个周期，理论上应该走完了,90分钟开始监控时间控制*/
					   if((TimeCurrent()-BuySellPosRecord[SymPos].TradeTime[6])>7200)
					   {
						  ticket =OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
					   ...
					   else if((TimeCurrent()-BuySellPosRecord[SymPos].TradeTime[6])>5400)
					   {   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
				...
				{   	   
				   ...
				   else
				   {
					...
					if(FiveM_StrongWeak<0.8)	
					{
						...
						vbid    = MarketInfo(MySymbol[SymPos],MODE_BID);
					   /*在非多头向上的情况下，一分钟120个周期，理论上应该走完了,90分钟开始监控时间控制*/
					   if((TimeCurrent()-BuySellPosRecord[SymPos].TradeTime[7])>7200)
					   {
						ticket =OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
					   ...
					   else if((TimeCurrent()-BuySellPosRecord[SymPos].TradeTime[7])>5400)
					   {   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
	...
	/*超过60美金，直接关掉所有订单，落袋为安，完成一次循环*/
	if(orderprofitall() > 60)
	{
		ordercloseall();
		Print("This turn Own more than 30 USD,Close all");
	}
