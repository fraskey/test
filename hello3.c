...
void initbuysellpos(int SymPos)
{
	...
	{	
		...
		BuySellPosRecord[SymPos].NextModifyValue[i] = 0;
...
//////////////////////////////////////////////////////////////////////////////
   ...
   for(SymPos = 0; SymPos < symbolNum;SymPos++)
   {
   	...
   	if (5 == Period() )
    {
      /*获取必要参数*/
				...
				{				
					if(((int)OrderMagicNumber()) == MakeMagic(SymPos,MagicNumberOne))
					{
						...
						if(orderStopless >OrderStopLoss() )
						{
							...
							Print(my_symbol+" MagicNumberOne1 Modify:" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
											+"MagicNumberOnePos="+BuySellPosRecord[SymPos].MagicNumberOnePos);										
							res=OrderModify(OrderTicket(),OrderOpenPrice(),
								   orderStopless,orderTakeProfit,0,clrPurple);
		...
		//突破型买点，在欧美交易时间交投活跃期间开突破类型单，防止假突破，回测非常大，非常难，在模拟盘上先优化
		//找到好的条件再启用
   		if((FourH_StrongWeak>0.8)&&(OneD_StrongWeak>0.8)&&0
			...
			&&(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberOne))==true))
   		{
			...
			&&(ThirtyM_StrongWeak > BoolCrossRecord[SymPos].CrossThirtyM_StrongWeak[3])
			...
			&&(0.8>BoolCrossRecord[SymPos].CrossThirtyM_StrongWeak[4]))			
   			{
				...
				Print(my_symbol+" MagicNumberOne1 OrderSend:" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
							+"MagicNumberOnePos="+BuySellPosRecord[SymPos].MagicNumberOnePos);		
			...
			&&(ThirtyM_StrongWeak >= BoolCrossRecord[SymPos].CrossThirtyM_StrongWeak[2])
			&&(ThirtyM_StrongWeak > BoolCrossRecord[SymPos].CrossThirtyM_StrongWeak[3])
			&&(ThirtyM_StrongWeak > BoolCrossRecord[SymPos].CrossThirtyM_StrongWeak[4])	
			...
			&&(0.8>BoolCrossRecord[SymPos].CrossThirtyM_StrongWeak[4]))	
				...
				*/
				...
				Print(my_symbol+" MagicNumberOne2 OrderSend:" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
							+"MagicNumberOnePos="+BuySellPosRecord[SymPos].MagicNumberOnePos);	
							
				ticket = OrderSend(my_symbol,OP_BUY,orderLots,orderPrice,3,orderStopless,orderTakeProfit,
							   my_symbol+"MagicNumberOne2",MakeMagic(SymPos,MagicNumberOne),0,Blue);
		...
		//转折型买点
			...
			&&(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberOne))==true))			
   		{
			...
			/*实际情况是当大周期多头向下时，小周期很暗短期改变，需要大止损，该买点还需测试*/
			...
			&&(0.8>BoolCrossRecord[SymPos].CrossThirtyM_StrongWeak[2]))				
   			{
				...
				orderStopless =MinValue3- bool_length; 	
				BuySellPosRecord[SymPos].NextModifyValue[0] = orderStopless;
				...
				*/
				...
				Print(my_symbol+" MagicNumberOne3 OrderSend:" + "orderLots=" + orderLots +"orderPrice ="
							+orderPrice+"orderStopless="+orderStopless
							+"MagicNumberOnePos="+BuySellPosRecord[SymPos].MagicNumberOnePos);	
									
				ticket = OrderSend(my_symbol,OP_BUY,orderLots,orderPrice,3,orderStopless,orderTakeProfit,
							   my_symbol+"MagicNumberOne3",MakeMagic(SymPos,MagicNumberOne),0,Blue);
	
				 if(ticket <0)
				 {
					Print("OrderSend MagicNumberOne3 failed with error #",GetLastError());
				 }
				 else
				 {      
					/*过20个周期再重新评估一下止损值，可放大止损值*/
					BuySellPosRecord[SymPos].NextModifyPos[0] = iBars(my_symbol,0)+15;			 
					BuySellPosRecord[SymPos].TradeTimePos[0] = iBars(MySymbol[SymPos],0)+120;				 				 
					Print("OrderSend MagicNumberOne3  successfully");
				 }													
				Sleep(1000);		
			...
			&&(0.8>BoolCrossRecord[SymPos].CrossThirtyM_StrongWeak[2]))				
				...
				*/
				...
				Print(my_symbol+" MagicNumberOne4 OrderSend:" + "orderLots=" + orderLots +"orderPrice ="
							+orderPrice+"orderStopless="+orderStopless
							+"MagicNumberOnePos="+BuySellPosRecord[SymPos].MagicNumberOnePos);	
									
				ticket = OrderSend(my_symbol,OP_BUY,orderLots,orderPrice,3,orderStopless,orderTakeProfit,
							   my_symbol+"MagicNumberOne4",MakeMagic(SymPos,MagicNumberOne),0,Blue);
	
				 if(ticket <0)
				 {
					Print("OrderSend MagicNumberOne4 failed with error #",GetLastError());
				 }
				 else
				 {     
					/*过20个周期再重新评估一下止损值，可放大止损值*/
					BuySellPosRecord[SymPos].NextModifyPos[0] = iBars(my_symbol,0)+15;					 
					BuySellPosRecord[SymPos].TradeTimePos[0] = iBars(MySymbol[SymPos],0)+120;				 				 
					Print("OrderSend MagicNumberOne4  successfully");
				 }													
				Sleep(1000);	
				...
				{				
					if(((int)OrderMagicNumber()) == MakeMagic(SymPos,MagicNumberTwo))
					{
						...
						if(orderStopless < OrderStopLoss() )
						{
							...
							Print(my_symbol+" MagicNumberTwo1 Modify:" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
		...
		//突破型卖点，在欧美交易时间交投活跃期间开突破类型单，防止假突破		
			...
			&&(0.2<BoolCrossRecord[SymPos].CrossThirtyM_StrongWeak[4]))				
   			{
				...
				Print(my_symbol+" MagicNumberTwo2 OrderSend:" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
							+"MagicNumberTwoPos="+BuySellPosRecord[SymPos].MagicNumberTwoPos);																 
				ticket = OrderSend(my_symbol,OP_SELL,orderLots,orderPrice,3,orderStopless,orderTakeProfit,
							   my_symbol+"MagicNumberTwo2",MakeMagic(SymPos,MagicNumberTwo),0,Blue);
		...
		//转折型卖点
			...
			&&(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberTwo))==true))
   		{
			...
			/*三十分钟周期多头向上时，慎重做空，一而鼓，再而竭，三而衰由止损保障，确保多头陷阱*/
   			if((4 == crossflag)&&(4==BoolCrossRecord[SymPos].CrossFlag[0])
			&&(4 ==BoolCrossRecord[SymPos].CrossFlag[2])
			...
			&&(0.2<BoolCrossRecord[SymPos].CrossThirtyM_StrongWeak[2]))
   			{
				...
				BuySellPosRecord[SymPos].NextModifyValue[1] = orderStopless;
				...
				*/
				...
				Print(my_symbol+" MagicNumberTwo3 OrderSend" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
									+"MagicNumberTwoPos="+BuySellPosRecord[SymPos].MagicNumberTwoPos);							
				 ticket = OrderSend(my_symbol,OP_SELL,orderLots,orderPrice,3,orderStopless,orderTakeProfit,
								   my_symbol+"MagicNumberTwo3",MakeMagic(SymPos,MagicNumberTwo),0,Blue);
		
				 if(ticket <0)
				 {
					Print("OrderSend MagicNumberTwo3 failed with error #",GetLastError());
				 }
				 else
				 {   
					/*过20个周期再重新评估一下止损值，可放大止损值*/
					BuySellPosRecord[SymPos].NextModifyPos[1] = iBars(my_symbol,0)+15;					 
					BuySellPosRecord[SymPos].TradeTimePos[1] =iBars(MySymbol[SymPos],0) +120;				 					 
					Print("OrderSend MagicNumberTwo3  successfully");
				 }
			...
			/*三十分钟线未明显多头向上，五分钟线背驰就认为是多头陷阱*/
				...
				*/
				...
				Print(my_symbol+" MagicNumberTwo4 OrderSend" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
									+"MagicNumberTwoPos="+BuySellPosRecord[SymPos].MagicNumberTwoPos);							
				 ticket = OrderSend(my_symbol,OP_SELL,orderLots,orderPrice,3,orderStopless,orderTakeProfit,
								   my_symbol+"MagicNumberTwo4",MakeMagic(SymPos,MagicNumberTwo),0,Blue);
		
				 if(ticket <0)
				 {
					Print("OrderSend MagicNumberTwo4 failed with error #",GetLastError());
				 }
				 else
				 {   
					/*过20个周期再重新评估一下止损值，可放大止损值*/
					BuySellPosRecord[SymPos].NextModifyPos[1] = iBars(my_symbol,0)+15;					 
					BuySellPosRecord[SymPos].TradeTimePos[1] = iBars(MySymbol[SymPos],0)+120;				 					 
					Print("OrderSend MagicNumberTwo4  successfully");
				 }
	    ...
	    //大周期多头市场，出现转空头转强迹象，轻仓做空博反弹
   		if((FourH_StrongWeak>0.8)
				&&(ThirtyM_StrongWeak<0.8)&&0)
   		{	  
	  
   			if((-5 == crossflag)&&(-5==BoolCrossRecord[SymPos].CrossFlag[0])
			//&&((BoolCrossRecord[SymPos].CrossBoolPos[1]-BuySellPosRecord[SymPos].MagicNumberTwoPos)>0)
			&&(-5 ==BoolCrossRecord[SymPos].CrossFlag[2])			
			&&(0.8<BoolCrossRecord[SymPos].CrossThirtyM_StrongWeak[4])
			&&(0.8<BoolCrossRecord[SymPos].CrossThirtyM_StrongWeak[5])
			&&(0.8<BoolCrossRecord[SymPos].CrossThirtyM_StrongWeak[6]))			
   			{
				vbid    = MarketInfo(my_symbol,MODE_BID);	
				vdigits = (int)MarketInfo(my_symbol,MODE_DIGITS);
				MaxValue4 = -1;
				...
				BuySellPosRecord[SymPos].TakeProfit[4] = orderTakeProfit;
				BuySellPosRecord[SymPos].RecentMax[4] = MaxValue4;
				
				//orderTakeProfit = 0;				
				

								
				if(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberFive))==true)
				{
					/*交易时间才开单*/
					if(opendaycheck(SymPos) == true)
					{		
						...
						Print(my_symbol+" MagicNumberFive1 OrderSend:" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
						...
						{				
							if(((int)OrderMagicNumber()) == MakeMagic(SymPos,MagicNumberFive))
							{
								...
								if(orderStopless < OrderStopLoss() )
								{
									...
									Print(my_symbol+" MagicNumberFive1 Modify:" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
													+"MagicNumberFivePos="+BuySellPosRecord[SymPos].MagicNumberFivePos);										
									res=OrderModify(OrderTicket(),OrderOpenPrice(),
										   orderStopless,orderTakeProfit,0,clrPurple);
									 ...
									 if(false == res)
									 {
										...
										Print("Error in MagicNumberFive1 OrderModify. Error code=",GetLastError());									
									 }
									 else
									 {            
										//BuySellPosRecord[SymPos].TradeTimePos[4] = iBars(MySymbol[SymPos],0);				 								 
										Print("OrderModify MagicNumberFive1  successfully ");
									 }								
	   ...
	   //大周期空头市场，出现转多头转强迹象，轻仓做多博反弹
		if((FourH_StrongWeak<0.2)
			&&(ThirtyM_StrongWeak>0.2)&&0)
		{
   			if((5 == crossflag)&&(5==BoolCrossRecord[SymPos].CrossFlag[0])
			//&&((BoolCrossRecord[SymPos].CrossBoolPos[1]-BuySellPosRecord[SymPos].MagicNumberTwoPos)>0)
			&&(5 ==BoolCrossRecord[SymPos].CrossFlag[2])			
			...
			&&(0.2>BoolCrossRecord[SymPos].CrossThirtyM_StrongWeak[6]))			
   			{
				...
				BuySellPosRecord[SymPos].TakeProfit[5] = orderTakeProfit;
				BuySellPosRecord[SymPos].RecentMin[5] = MinValue3;
				
				//orderTakeProfit = 0;

				if(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberSix))==true)
				{
					/*交易时间才开单*/
					if(opendaycheck(SymPos) == true)
					{						
								...
								if(orderStopless >OrderStopLoss() )
								{
									...
									res=OrderModify(OrderTicket(),OrderOpenPrice(),
									 ...
									 if(false == res)
									 {
										...
										Print("Error in MagicNumberSix1 OrderModify. Error code=",GetLastError());									
									 }
									 else
									 {         
										//BuySellPosRecord[SymPos].TradeTimePos[5] = iBars(MySymbol[SymPos],0);				 								 
										Print("OrderModify MagicNumberSix1  successfully ");
									 }								
      ...
      /*获取必要参数*/
		...
		if((5 == crossflag)&&(5==BoolCrossRecord[SymPos].CrossFlag[0])
   		...
   		//大周期处于多头市场，本周期在下跌背驰阶段买入，趋势交易，目的是为了找到比较好的入场点，和止损点
		//突破型买点，在欧美交易时间交投活跃期间开突破类型单，防止假突破，采用三重多头测试
			...
			&&(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberThree))==true))
   		{
			...
			&&(4 !=BoolCrossRecord[SymPos].CrossFlag[3])
			&&(FiveM_StrongWeak > BoolCrossRecord[SymPos].CrossFiveM_StrongWeak[3])
			...
			&&(0.8>BoolCrossRecord[SymPos].CrossFiveM_StrongWeak[4]))					
				...
				*/
				...
				Print(my_symbol+" MagicNumberThree1 OrderSend:" + "orderLots=" + orderLots +"orderPrice ="
							+orderPrice+"orderStopless="+orderStopless
							+"MagicNumberThreePos="+BuySellPosRecord[SymPos].MagicNumberThreePos);	
									
				ticket = OrderSend(my_symbol,OP_BUY,orderLots,orderPrice,3,orderStopless,orderTakeProfit,
							   my_symbol+"MagicNumberThree1",MakeMagic(SymPos,MagicNumberThree),0,Blue);
			...
			&&(0.8>BoolCrossRecord[SymPos].CrossFiveM_StrongWeak[2]))				
   			{
				...
				BuySellPosRecord[SymPos].NextModifyValue[2] = orderStopless;
				orderStopless =MinValue3- bool_length; 	
				...
				*/
				...
				ticket = OrderSend(my_symbol,OP_BUY,orderLots,orderPrice,3,orderStopless,orderTakeProfit,
				 ...
				 {     
					/*过20个周期再重新评估一下止损值，可放大止损值*/
					BuySellPosRecord[SymPos].NextModifyPos[2] = iBars(my_symbol,0)+15;					 
					BuySellPosRecord[SymPos].TradeTimePos[2] = iBars(MySymbol[SymPos],0)+120;				 				 
		...
		if((-5 == crossflag)&&(-5==BoolCrossRecord[SymPos].CrossFlag[0])
   		...
   		//大周期处于空头市场，本周期在上涨背驰阶段卖出，趋势交易，目的是为了找到比较好的入场点，和止损点
		//突破型卖点，在欧美交易时间交投活跃期间开突破类型单，防止假突破，采用三重空头测试	
			...
			&&(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberFour))==true))
   		{
			...
			&&(-4 !=BoolCrossRecord[SymPos].CrossFlag[3])
			&&(ThirtyM_StrongWeak < BoolCrossRecord[SymPos].CrossFiveM_StrongWeak[3])			
			&&(ThirtyM_StrongWeak < BoolCrossRecord[SymPos].CrossFiveM_StrongWeak[4])			
			...
			&&(0.2<BoolCrossRecord[SymPos].CrossFiveM_StrongWeak[4]))	
		
   			{
					...
					*/
					...
					Print(my_symbol+" MagicNumberFour1 OrderSend" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
										+"MagicNumberFourPos="+BuySellPosRecord[SymPos].MagicNumberFourPos);							
					 ticket = OrderSend(my_symbol,OP_SELL,orderLots,orderPrice,3,orderStopless,orderTakeProfit,
									   my_symbol+"MagicNumberFour1",MakeMagic(SymPos,MagicNumberFour),0,Blue);
		...
		//转折型卖点
			...
			&&(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberFour))==true))
   		{
			...
			&&(0.2<BoolCrossRecord[SymPos].CrossFiveM_StrongWeak[2]))
   			{
				...
				BuySellPosRecord[SymPos].NextModifyValue[3] = orderStopless;
				...
				*/
				...
				Print(my_symbol+" MagicNumberFour3 OrderSend" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
				 ...
				 {   
					/*过20个周期再重新评估一下止损值，可放大止损值*/
					BuySellPosRecord[SymPos].NextModifyPos[3] = iBars(my_symbol,0)+15;					 
					BuySellPosRecord[SymPos].TradeTimePos[3] = iBars(MySymbol[SymPos],0)+120;				 					 
			...
			{	  
				...
				&&(0.8<BoolCrossRecord[SymPos].CrossFiveM_StrongWeak[6]))			
				{
					vbid    = MarketInfo(my_symbol,MODE_BID);	
					vdigits = (int)MarketInfo(my_symbol,MODE_DIGITS);
					MaxValue4 = -1;
				...
				BuySellPosRecord[SymPos].TakeProfit[6] = orderTakeProfit;
				BuySellPosRecord[SymPos].RecentMax[6] = MaxValue4;
				...
				if(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberSeven))==true)
				{
						
					Print(my_symbol+" MagicNumberSeven1 OrderSend:" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
					 ...
					 if(ticket <0)
					 {
						Print("OrderSend MagicNumberSeven1 failed with error #",GetLastError());
					 }
					 else
					 {          
						BuySellPosRecord[SymPos].TradeTimePos[6] = iBars(MySymbol[SymPos],0);				 				 
						Print("OrderSend MagicNumberSeven1  successfully");
					 }
				}
				else
				{
						...
						{				
							if(((int)OrderMagicNumber()) == MakeMagic(SymPos,MagicNumberSeven))
							{
								...
								if(orderStopless < OrderStopLoss() )
								{
									...
									Print(my_symbol+" MagicNumberSeven1 Modify:" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
													+"MagicNumberSevenPos="+BuySellPosRecord[SymPos].MagicNumberSevenPos);										
									res=OrderModify(OrderTicket(),OrderOpenPrice(),
										   orderStopless,orderTakeProfit,0,clrPurple);
	   ...
	   //大周期空头市场，出现转多头转强迹象，轻仓做多博反弹
			if((ThirtyM_StrongWeak<0.2)
				&&(FiveM_StrongWeak>0.2)&&0)
		{
   			if((5 == crossflag)&&(5==BoolCrossRecord[SymPos].CrossFlag[0])
			//&&((BoolCrossRecord[SymPos].CrossBoolPos[1]-BuySellPosRecord[SymPos].MagicNumberEightPos)>0)
			&&(5 ==BoolCrossRecord[SymPos].CrossFlag[2])			
			&&(0.2>BoolCrossRecord[SymPos].CrossFiveM_StrongWeak[4])
			&&(0.2>BoolCrossRecord[SymPos].CrossFiveM_StrongWeak[5])
			&&(0.2>BoolCrossRecord[SymPos].CrossFiveM_StrongWeak[6]))			
   			{
				...
				BuySellPosRecord[SymPos].TakeProfit[7] = orderTakeProfit;
				BuySellPosRecord[SymPos].RecentMin[7] = MinValue3;
				...
				if(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberEight))==true)
				{
					
					Print(my_symbol+" MagicNumberEight1 OrderSend:" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
					...
					ticket = OrderSend(my_symbol,OP_BUY,orderLots,orderPrice,3,orderStopless,orderTakeProfit,
					 ...
					 if(ticket <0)
					 {
						Print("OrderSend MagicNumberEight1 failed with error #",GetLastError());
					 }
					 else
					 {            
				 		BuySellPosRecord[SymPos].TradeTimePos[7] = iBars(MySymbol[SymPos],0);				 
						Print("OrderSend MagicNumberEight1  successfully");
					 }
				}
				else
				{
   ...
   ////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if(NowMagicNumber == MagicNumberOne)
			{
				...
				{   
					...
					if((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[0]) > 0)
					{
						...
						if(orderStopless <BuySellPosRecord[SymPos].NextModifyValue[0])
						{
							orderStopless = BuySellPosRecord[SymPos].NextModifyValue[0];
						}						
					...
					if((ThirtyM_StrongWeak<0.8)||(FourH_StrongWeak<0.8))
					{
					   ...
					   /*在非多头向上的情况下，五分钟210个周期，理论上应该走完了,180周期开始监控时间控制*/
					   if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[0])>210)
					   {
						ticket =OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
					   ...
					   else if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[0])>180)
					   {   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
					...
					else if((FourH_StrongWeak>0.8)&&(ThirtyM_StrongWeak>0.8))	
					{
					...
					if(ThirtyM_StrongWeak<0.2)
					{
						...
						if(ticket <0)
						{
							Print("OrderClose MagicNumberOne33  FourH_StrongWeak failed with error #",GetLastError());
						}
						else
						{            
							Print("OrderClose MagicNumberOne33  FourH_StrongWeak successfully");
						}    
						Sleep(1000); 	
						*/
					}
				...
				{   	   
				   ...
				   else
				   {
					...
					if((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[1]) > 0)
					{
						...
						if(orderStopless > BuySellPosRecord[SymPos].NextModifyValue[1])
						{
							orderStopless = BuySellPosRecord[SymPos].NextModifyValue[1];
						}	
					...
					if((ThirtyM_StrongWeak>0.2)||(FourH_StrongWeak>0.2))
					{
						...
						if(OrderTakeProfit() < 0.1)
						{
							...
							orderTakeProfit = BuySellPosRecord[SymPos].TakeProfit[1];
							orderTakeProfit = NormalizeDouble(orderTakeProfit,vdigits);
						...
						/*在非多头向下的情况下五分钟210个周期，理论上应该走完了,180分钟开始监控时间控制*/
						if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[1])>210)
						{
						  ticket =OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
						...
						else if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[1])>180)
						{   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
					...
					if(ThirtyM_StrongWeak > 0.8)
					{
						...
						vask    = MarketInfo(MySymbol[SymPos],MODE_ASK);
						 ...
						 if(ticket <0)
						 {
							Print("OrderClose MagicNumberTwo33 FourH_StrongWeak failed with error #",GetLastError());
						 }
						 else
						 {            
							Print("OrderClose MagicNumberTwo33 FourH_StrongWeak  successfully");
						 }       
						Sleep(1000); 						
						*/
					}					
				...
				{   
				   ...
				   else
				   {
					...
					if((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[2]) > 0)
					{
						...
						if(orderStopless <BuySellPosRecord[SymPos].NextModifyValue[2])
						{
							orderStopless = BuySellPosRecord[SymPos].NextModifyValue[2];
						}						
					...
					if((FiveM_StrongWeak<0.8)||(ThirtyM_StrongWeak<0.8))
					{
					   ...
					   /*在非多头向上的情况下，一分钟210个周期，理论上应该走完了,180周期开始监控时间控制*/
					   if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[2])>210)
					   {
						ticket =OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
					   ...
					   else if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[2])>180)
					   {   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
				...
				{   	   
				   ...
				   else
				   {
					...
					if((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[3]) > 0)
					{
						...
						orderPrice = vbid;						 
						orderStopless =MaxValue4 + 2*bool_length; 	

						if(orderStopless > BuySellPosRecord[SymPos].NextModifyValue[3])
						{
							orderStopless = BuySellPosRecord[SymPos].NextModifyValue[3];
						}	
					...
					if((FiveM_StrongWeak>0.2)||(ThirtyM_StrongWeak>0.2))
					{
						...
						/*在非多头向下的情况下一分钟210个周期，理论上应该走完了,180分钟开始监控时间控制*/
						if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[3])>210)
						{
						  ticket =OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
						...
						else if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[3])>180)
						{   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
			...
			if(NowMagicNumber == MagicNumberFive)
			{
				...
				{   	   
					...
					if(ThirtyM_StrongWeak>0.2)	
					{
						...
						vask    = MarketInfo(MySymbol[SymPos],MODE_ASK);
					   /*在非多头向下的情况下五分钟210个周期，理论上应该走完了,180分钟开始监控时间控制*/
					   if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[4])>210)
					   {
						  ticket =OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
						  
						 if(ticket <0)
						 {
							Print("OrderClose MagicNumberSeven 111 failed with error #",GetLastError());
						 }
						 else
						 {            
							Print("OrderClose MagicNumberSeven 111  successfully");
						 }    
						 Sleep(1000);  	   
					   }
					   
					   else if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[4])>180)
					   {   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
							 if(ticket <0)
							 {
								Print("OrderClose MagicNumberSeven 222 failed with error #",GetLastError());
							 }
							 else
							 {            
								Print("OrderClose MagicNumberSeven 222  successfully");
							 }    
							 Sleep(1000);     	           
						  }   	   
					   }  				   
				...
				{   	   
				   ...
				   else
				   {
					...
					if(ThirtyM_StrongWeak<0.8)	
					{
						...
						vbid    = MarketInfo(MySymbol[SymPos],MODE_BID);
					   /*在非多头向上的情况下，五分钟210个周期，理论上应该走完了,180分钟开始监控时间控制*/
					   if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[5])>210)
					   {
						ticket =OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
						  
						 if(ticket <0)
						 {
							Print("OrderClose MagicNumberEight 111 failed with error #",GetLastError());
						 }
						 else
						 {            
							Print("OrderClose MagicNumberEight 111  successfully");
						 }    
						 Sleep(1000);  	   
					   }
					   
					   else if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[5])>180)
					   {   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
							 if(ticket <0)
							 {
								Print("OrderClose MagicNumberEight 222 failed with error #",GetLastError());
							 }
							 else
							 {            
								Print("OrderClose MagicNumberEight 222  successfully");
							 }    
							 Sleep(1000);     	           
						  }   	   
					   }  				   
				...
				{   	   
				   ...
				   else
				   {
					...
					if(FiveM_StrongWeak>0.2)	
					{
						...
						vask    = MarketInfo(MySymbol[SymPos],MODE_ASK);
					   /*在非多头向下的情况下一分钟210个周期，理论上应该走完了,180分钟开始监控时间控制*/
					   if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[6])>210)
					   {
						  ticket =OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
					   ...
					   else if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[6])>180)
					   {   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
				...
				{   	   
				   ...
				   else
				   {
					...
					if(FiveM_StrongWeak<0.8)	
					{
						...
						vbid    = MarketInfo(MySymbol[SymPos],MODE_BID);
					   /*在非多头向上的情况下，一分钟210个周期，理论上应该走完了,180分钟开始监控时间控制*/
					   if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[7])>210)
					   {
						ticket =OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
					   ...
					   else if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[7])>180)
					   {   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
	...
	/*超过50美金，直接关掉所有盈利订单，落袋为安，完成一次循环*/
	if(orderprofitall() > 50)
	{
		
		ordercloseallwithprofit(2);
		Print("1、This turn Own more than 50 USD,Close all");
	}

	/*三个以上10美金订单，直接关掉所有盈利订单，落袋为安，完成一次循环*/
	if(ordercountwithprofit(10)>= 3)
	{
		
		ordercloseallwithprofit(3);
		Print("2、This turn Own more than three 10 USD,Close all");
	}

	/*两个以上15美金订单，直接关掉所有盈利订单，落袋为安，完成一次循环*/
	if(ordercountwithprofit(15)>= 2)
	{
		
		ordercloseallwithprofit(5);
		Print("3、This turn Own more than two 15 USD,Close all");
	}

	/*一个以上20美金订单，直接关掉所有盈利订单，落袋为安，完成一次循环*/
	if(ordercountwithprofit(20)>= 1)
	{		
		ordercloseallwithprofit(5);
		Print("4、This turn Own more than one 20 USD,Close all");
	}

	/*订单数量4个，且获利超过30美元，落袋为安*/
	if((OrdersTotal()==4)&&(orderprofitall()>30))
	{
		ordercloseallwithprofit(2);		
		Print("5、This turn Own more than one 30 USD,equal 4 order Close all");		
	}	
	
	/*订单数量3个，且获利超过20美元，落袋为安*/
	if((OrdersTotal()==3)&&(orderprofitall()>20))
	{
		ordercloseallwithprofit(2);		
		Print("6、This turn Own more than one 20 USD,equal 3 order Close all");		
	}
	
	/*订单数量2个，且获利超过15美元，落袋为安*/
	if((OrdersTotal()==2)&&(orderprofitall()>15))
	{
		ordercloseallwithprofit(2);		
		Print("7、This turn Own more than one 15 USD,equal 2 order Close all");		
	}
