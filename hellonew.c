...
void initbuysellpos(int SymPos)
{
	...
	BuySellPosRecord[SymPos].MagicNumberThirteenPos = BoolCrossRecord[SymPos].CrossBoolPos[0];
	BuySellPosRecord[SymPos].MagicNumberFourteenPos = BoolCrossRecord[SymPos].CrossBoolPos[0];
	BuySellPosRecord[SymPos].MagicNumberFifteenPos = BoolCrossRecord[SymPos].CrossBoolPos[0];
	BuySellPosRecord[SymPos].MagicNumberSixteenPos = BoolCrossRecord[SymPos].CrossBoolPos[0];		
	 ...
	 for(i = 0; i < symbolNum;i++)
	 {
		...
		else if(240 == Period() )
		{
		 ...
		 else if (1 == Period() )
		 {
			  ...
			  if(GlobalVariableCheck("g_OneM_BF"+MySymbol[i]) == TRUE)
			  {  
					GlobalVariableSet("g_OneM_BF"+MySymbol[i],0.5);
				  OneM_BoolFlag = GlobalVariableGet("g_OneM_BF"+MySymbol[i]);    
				  Print("g_OneM_BoolFlag already exist  = "+MySymbol[i]+DoubleToString(OneM_BoolFlag));        
			  }
			  else
			  {

					GlobalVariableSet("g_OneM_BF"+MySymbol[i],0.5);
				  if(GlobalVariableCheck("g_OneM_BF"+MySymbol[i]) == FALSE)
				  {
					Print("init False due to g_OneM_BoolFlag set false!"+MySymbol[i]);  
					return -1;     		      	  
				  }		    
				  else
				  {
					OneM_BoolFlag = GlobalVariableGet("g_OneM_BF"+MySymbol[i]);  
						Print("init g_OneM_BoolFlag is OK  = "+MySymbol[i]+DoubleToString(OneM_BoolFlag));  		          			      	  
				  }  
...
int deinit()
{
   ...
   {   
      ...
      else if (1 == Period() )
      {
         ...
         if(GlobalVariableCheck("g_OneM_BF"+MySymbol[i]) == TRUE)
         {      
             GlobalVariableDel("g_OneM_BF"+MySymbol[i]);
      ...
      /*获取必要参数*/
						...
						||(((int)OrderMagicNumber()) == MakeMagic(SymPos,MagicNumberOne)))			
					{
						...
						if(orderStopless >OrderStopLoss() )
						{
							...
							Print(my_symbol+" MagicNumberThree/One Modify:" + "orderLots=" + orderLots +"orderPrice ="
											...
											+"MagicNumberThree/One="+BuySellPosRecord[SymPos].MagicNumberThreePos);									
				...
				{					
					ticket = OrderSend(my_symbol,OP_BUY,orderLots,orderPrice,3,orderStopless,orderTakeProfit,
					 ...
					 { 
						...
						BuySellPosRecord[SymPos].NextModifyPos[0] = iBars(my_symbol,0)+22;					 
		...
		//转折型买点
			...
			&&((OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberThree))==true)&&(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberOne))==true)))
   		{
			...
			&&(-1 ==BoolCrossRecord[SymPos].CrossFlag[4])	
				...
				if(true == accountcheck())
				{
					ticket = OrderSend(my_symbol,OP_BUY,orderLots,orderPrice,3,orderStopless,orderTakeProfit,
					 ...
					 {     
						...
						FiveM_Freq++;
						ThirtyM_Freq++;	
						BuySellPosRecord[SymPos].NextModifyPos[2] = iBars(my_symbol,0)+22;					 
			...
			/*五分钟非多头向下，一分钟bool背驰，空头陷阱*/
   			if((-4 == crossflag)&&(-4==BoolCrossRecord[SymPos].CrossFlag[0])
			&&(-1 ==BoolCrossRecord[SymPos].CrossFlag[2])
				...
				{		
...

					{
						...
						if(orderStopless < OrderStopLoss() )
						{
							...
							Print(my_symbol+" MagicNumberFour/Two Modify:" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
			...
			&&((OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberFour))==true)&&(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberTwo))==true)))
   		{
			...
			&&(0.2<BoolCrossRecord[SymPos].CrossFiveM_StrongWeak[4]))	
				...
				*/
				...
				Print(my_symbol+" MagicNumberTwo OrderSend" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
									+"MagicNumberFourPos="+BuySellPosRecord[SymPos].MagicNumberFourPos);							
				 if(true == accountcheck())
				 {
					 ...
					 {   
						...
						FiveM_Freq++;
						ThirtyM_Freq++;	
						BuySellPosRecord[SymPos].NextModifyPos[1] = iBars(my_symbol,0)+22;					 
				...
				Print(my_symbol+" MagicNumberFour3 OrderSend" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
				 ...
				 if(true == accountcheck())
				 {
					 ...
					 {   
						...
						FiveM_Freq++;
						ThirtyM_Freq++;	
						BuySellPosRecord[SymPos].NextModifyPos[3] = iBars(my_symbol,0)+22;					 
						BuySellPosRecord[SymPos].TradeTimePos[3] = iBars(MySymbol[SymPos],0);				 					 
      ...
      /*获取必要参数*/
						...
						||(((int)OrderMagicNumber()) == MakeMagic(SymPos,MagicNumberThirteen)))					
					{
						...
						if(orderStopless >OrderStopLoss() )
						{
							...
							Print(my_symbol+" MagicNumberFive/Seven/Thirteen Modify:" + "orderLots=" + orderLots +"orderPrice ="
											+orderPrice+"orderStopless="+orderStopless
											+"MagicNumberFive/Seven/Thirteen="+BuySellPosRecord[SymPos].MagicNumberThreePos);									
		...
		//突破型买点，在欧美交易时间交投活跃期间开突破类型单，防止假突破，采用三重多头测试，最关键的一点还是要设置小止损
			...
			&&((OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberSeven))==true)&&(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberFive))==true)))
   		{
   			...
   			if(/*(-4 == crossflag)&&*/(-4==BoolCrossRecord[SymPos].CrossFlag[0])
			&& (-4 !=BoolCrossRecord[SymPos].CrossFlag[4])
			&&((OneM_BoolFlag >0.1)&&(OneM_BoolFlag < 3))				
			...
			&&(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberThirteen))==true)))
   		{
			...
			/*三十分钟多头向上，五分钟上涨两次确认，回调空头陷阱*/
				...
				*/
				...
				Print(my_symbol+" MagicNumberThirteen OrderSend" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
									+"MagicNumberThirteenPos="+BuySellPosRecord[SymPos].MagicNumberThirteenPos);							
				 ...
				 if(true == accountcheck())
				 {
					 ...
					 {   
						...
						FiveM_Freq++;
						ThirtyM_Freq++;				 
						BuySellPosRecord[SymPos].NextModifyPos[12] = iBars(my_symbol,0)+20;					 
						BuySellPosRecord[SymPos].TradeTimePos[12] = iBars(MySymbol[SymPos],0);				 					 
				...
				{				
						...
						||(((int)OrderMagicNumber()) == MakeMagic(SymPos,MagicNumberFourteen)))					
					{
						...
						if(orderStopless < OrderStopLoss() )
						{
							...
							Print(my_symbol+" MagicNumberEight/Six/Fourteen Modify:" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
											+"MagicNumberEight/Six/Fourteen="+BuySellPosRecord[SymPos].MagicNumberFourPos);	
			...
			&&((OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberEight))==true)&&(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberSix))==true)))
   		{
			...
			&&(0.2<BoolCrossRecord[SymPos].CrossThirtyM_StrongWeak[4]))	
				...
				*/
				...
				Print(my_symbol+" MagicNumberSix OrderSend" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
			...
			&&(0.8>BoolCrossRecord[SymPos].CrossThirtyM_StrongWeak[2]))				
				...
				*/
				...
				Print(my_symbol+" MagicNumberFourteen OrderSend:" + "orderLots=" + orderLots +"orderPrice ="
							+orderPrice+"orderStopless="+orderStopless
							+"MagicNumberFourteenPos="+BuySellPosRecord[SymPos].MagicNumberFourteenPos);	
				...
				if(true == accountcheck())
				{
					...
					ticket = OrderSend(my_symbol,OP_SELL,orderLots,orderPrice,3,orderStopless,orderTakeProfit,
					 ...
					 {     
						...
						FiveM_Freq++;
						ThirtyM_Freq++;	
						BuySellPosRecord[SymPos].NextModifyPos[13] = iBars(my_symbol,0)+20;					 
						BuySellPosRecord[SymPos].TradeTimePos[13] = iBars(MySymbol[SymPos],0);				 				 
      ...
      /*获取必要参数*/
						...
						||(((int)OrderMagicNumber()) == MakeMagic(SymPos,MagicNumberFifteen)))					
					{
						...
						if(orderStopless >OrderStopLoss() )
						{
							...
							Print(my_symbol+" MagicNumberEleven/Nine/Fiifteen Modify:" + "orderLots=" + orderLots +"orderPrice ="
											+orderPrice+"orderStopless="+orderStopless
											+"MagicNumberEleven/Nine/Fiifteen="+BuySellPosRecord[SymPos].MagicNumberElevenPos);									
		...
		//突破型买点，在欧美交易时间交投活跃期间开突破类型单，防止假突破，采用三重多头测试，最关键的一点还是要设置小止损
			...
			&&((OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberEleven))==true)&&(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberNine))==true)))
   		{
   			...
   			if(/*(-4 == crossflag)&&*/(-4==BoolCrossRecord[SymPos].CrossFlag[0])
			&& (-4 !=BoolCrossRecord[SymPos].CrossFlag[4])
			&&((FiveM_BoolFlag >0.1)&&(FiveM_BoolFlag < 3))			
			...
			&&(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberFifteen))==true)))
   		{
			...
			/*四小时多头向上，三十分钟上涨两次确认，回调空头陷阱*/
				...
				*/
				...
				Print(my_symbol+" MagicNumberFifteen OrderSend" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
									+"MagicNumberFifteenPos="+BuySellPosRecord[SymPos].MagicNumberFifteenPos);							
				 ...
				 if(true == accountcheck())
				 {
					 ...
					 {   
						...
						FiveM_Freq++;
						ThirtyM_Freq++;				 
						BuySellPosRecord[SymPos].NextModifyPos[14] = iBars(my_symbol,0)+20;					 
						BuySellPosRecord[SymPos].TradeTimePos[14] = iBars(MySymbol[SymPos],0);				 					 
				...
				{				
						...
						||(((int)OrderMagicNumber()) == MakeMagic(SymPos,MagicNumberSixteen)))					
					{
						...
						if(orderStopless < OrderStopLoss() )
						{
							...
							Print(my_symbol+" MagicNumberTwelve/Ten/Sixteen Modify:" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
											+"MagicNumberTwelve/Ten/Sixteen="+BuySellPosRecord[SymPos].MagicNumberTwelvePos);	
			...
			&&((OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberTwelve))==true)&&(OneMOrderCloseStatus(MakeMagic(SymPos,MagicNumberTen))==true)))
   		{
			...
			&&(0.2<BoolCrossRecord[SymPos].CrossFourH_StrongWeak[4]))	
				...
				*/
				...
				Print(my_symbol+" MagicNumberTen OrderSend" + "orderLots=" + orderLots +"orderPrice ="+	 orderPrice+"orderStopless="+orderStopless
			...
			&&(0.8>BoolCrossRecord[SymPos].CrossFourH_StrongWeak[2]))				
				...
				*/
				...
				Print(my_symbol+" MagicNumberSixteen OrderSend:" + "orderLots=" + orderLots +"orderPrice ="
							+orderPrice+"orderStopless="+orderStopless
							+"MagicNumberSixteenPos="+BuySellPosRecord[SymPos].MagicNumberSixteenPos);	
				...
				if(true == accountcheck())
				{
					...
					ticket = OrderSend(my_symbol,OP_SELL,orderLots,orderPrice,3,orderStopless,orderTakeProfit,
					 ...
					 {     
						...
						FiveM_Freq++;
						ThirtyM_Freq++;	
						BuySellPosRecord[SymPos].NextModifyPos[15] = iBars(my_symbol,0)+20;					 
						BuySellPosRecord[SymPos].TradeTimePos[15] = iBars(MySymbol[SymPos],0);				 				 
       ...
       {	
			...
			if(NowMagicNumber == MagicNumberOne)
			{
				...
				{   
					...
					if((4 == BoolCrossRecord[SymPos].CrossFlag[0])&&( ChartEvent != iBars(NULL,0))
						&&(BoolCrossRecord[SymPos].CrossFlagTempPre != BoolCrossRecord[SymPos].CrossFlag[0])
			...
			if(NowMagicNumber == MagicNumberThree)
			{
				...
				{   
				   ...
				   else
				   {
					...
					if((4 == BoolCrossRecord[SymPos].CrossFlag[0])&&0)
					{
					...
					if((FiveM_StrongWeak<0.8)||(ThirtyM_StrongWeak<0.8))
					{
					   ...
					   if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[2])>400)
					   {
						ticket =OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
					   ...
					   else if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[2])>360)
					   {   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
				...
				{   	   
				   ...
				   else
				   {
					...
					if((FiveM_StrongWeak>0.2)||(ThirtyM_StrongWeak>0.2))
					{
						...
						if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[3])>400)
						{
						  ticket =OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
						...
						else if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[3])>360)
						{   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
			...
			if(NowMagicNumber == MagicNumberFive)
			{
				...
				{   
					...
					else if(((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[4]) >-50)
						&&((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[4]) <=-15)
						&&(-4.5 > OneM_BoolFlag))
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
						 if(ticket <0)
					...
					else if(((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[4]) >-15)
						&&(4.5 > BoolCrossRecord[SymPos].CrossFlag[0])
						&&(-4.5 > OneM_BoolFlag))					
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
					...
					if((-0.1 > BoolCrossRecord[SymPos].CrossFlag[0])
						&&(4.5 > ThirtyM_BoolFlag)
						&&((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[4]) < -1000))					
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
						 if(ticket <0)
						 {
			...
			if(NowMagicNumber == MagicNumberSix)
			{
				...
				{   	   
				   ...
				   else
				   {
					...
					else if(((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[5]) >-50)
						&&((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[5]) <=-15)
						&&(4.5 < OneM_BoolFlag))
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
						 if(ticket <0)
					...
					else if(((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[5]) >-15)
						&&(-4.5 < BoolCrossRecord[SymPos].CrossFlag[0])
						&&(4.5 < OneM_BoolFlag))	
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
					...
					if((0.1 < BoolCrossRecord[SymPos].CrossFlag[0])
						&&(-4.5 < ThirtyM_BoolFlag)
						&&((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[5]) < -1000))					
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
						 if(ticket <0)
						 {
			...
			if(NowMagicNumber == MagicNumberSeven)
			{
				...
				{   
				   ...
				   else
				   {
					...
					else if(((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[6]) >-20)
						&&(-4.5 > BoolCrossRecord[SymPos].CrossFlag[0]))
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
						 if(ticket <0)
					...
					if((ThirtyM_StrongWeak<0.8)||(FourH_StrongWeak<0.8))
					{
					   ...
					   /*在非多头向上的情况下，一分钟400个周期，理论上应该走完了,360周期开始监控时间控制*/
					   if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[6])>400)
					   {
						ticket =OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
					   ...
					   else if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[6])>360)
					   {   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
			...
			if(NowMagicNumber == MagicNumberEight)
			{
				...
				{   	   
				   ...
				   else
				   {
					...
					else if(((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[7]) >-20)
						&&(4.5 < BoolCrossRecord[SymPos].CrossFlag[0]))
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
						 if(ticket <0)
					...
					if((ThirtyM_StrongWeak>0.2)||(FourH_StrongWeak>0.2))
					{
						...
						/*在非多头向下的情况下一分钟400个周期，理论上应该走完了,360分钟开始监控时间控制*/
						if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[7])>400)
						{
						  ticket =OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
						...
						else if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[7])>360)
						{   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
			...
			if(NowMagicNumber == MagicNumberNine)
			{
				...
				{   
					...
					else if(((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[8]) >-50)
						&&((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[8]) <=-15)
						&&(-4.5 > FiveM_BoolFlag))
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
						 if(ticket <0)
					...
					else if(((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[8]) >-15)
						&&(4.5 > BoolCrossRecord[SymPos].CrossFlag[0])
						&&(-4.5 > FiveM_BoolFlag))					
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
					...
					if((-0.1 > BoolCrossRecord[SymPos].CrossFlag[0])
						&&(4.5 > FourH_BoolFlag)
						&&((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[8]) < -1000))					
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
						 if(ticket <0)
						 {
			...
			if(NowMagicNumber == MagicNumberTen)
			{
				...
				{   	   
				   ...
				   else
				   {
					...
					else if(((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[9]) >-50)
						&&((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[9]) <=-15)
						&&(4.5 < FiveM_BoolFlag))
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
						 if(ticket <0)
					...
					else if(((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[9]) >-15)
						&&(-4.5 < BoolCrossRecord[SymPos].CrossFlag[0])
						&&(4.5 < FiveM_BoolFlag))	
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
					...
					if((0.1 < BoolCrossRecord[SymPos].CrossFlag[0])
						&&(-4.5 < FourH_BoolFlag)
						&&((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[9]) < -1000))					
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
						 if(ticket <0)
						 {
			...
			if(NowMagicNumber == MagicNumberEleven)
			{
				...
				{   
				   ...
				   else
				   {
					...
					else if(((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[10]) >-20)
						&&(-4.5 > BoolCrossRecord[SymPos].CrossFlag[0]))
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
						 if(ticket <0)
					...
					if((FourH_StrongWeak<0.8)||(OneD_StrongWeak<0.8))
					{
					   ...
					   /*在非多头向上的情况下，一分钟400个周期，理论上应该走完了,360周期开始监控时间控制*/
					   if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[10])>400)
					   {
						ticket =OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
					   ...
					   else if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[10])>360)
					   {   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
			...
			if(NowMagicNumber == MagicNumberTwelve)
			{
				...
				{   	   
				   ...
				   else
				   {
					...
					else if(((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[11]) >-20)
						&&(4.5 < BoolCrossRecord[SymPos].CrossFlag[0]))
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
						 if(ticket <0)
					...
					if((FourH_StrongWeak>0.2)||(OneD_StrongWeak>0.2))
					{
						...
						/*在非多头向下的情况下一分钟400个周期，理论上应该走完了,360分钟开始监控时间控制*/
						if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[11])>400)
						{
						  ticket =OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
						...
						else if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[11])>360)
						{   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
			...
			if(NowMagicNumber == MagicNumberThirteen)
			{
				...
				{   
				   ...
				   else
				   {
					...
					if((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[12]) > 0)
					{
						
						/*初始化一个超大值，该值不可能达到*/
						BuySellPosRecord[SymPos].NextModifyPos[12] = iBars(MySymbol[SymPos],0)+10000000;	
						MinValue3 = 100000;
						for (i= 0;i < (iBars(my_symbol,0) -BoolCrossRecord[SymPos].CrossBoolPos[1]+5);i++)
						{
							if(MinValue3 > iLow(my_symbol,0,i))
					...
					else if(((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[12]) >-50)
						&&((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[12]) <=-15)
						&&(-4.5 > OneM_BoolFlag))
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
						 if(ticket <0)
					...
					else if(((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[12]) >-15)
						&&(4.5 > BoolCrossRecord[SymPos].CrossFlag[0])
						&&(-4.5 > OneM_BoolFlag))					
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
					...
					if((-0.1 > BoolCrossRecord[SymPos].CrossFlag[0])
						&&(4.5 > ThirtyM_BoolFlag)
						&&((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[12]) < -1000))					
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
						 if(ticket <0)
						 {
					...
					if((ThirtyM_StrongWeak<0.8) || (FourH_StrongWeak < 0.8))
					{
					   ...
					   /*在非多头向上的情况下，一分钟120个周期，理论上应该走完了,90周期开始监控时间控制*/
					   if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[12])>120)
					   {
						ticket =OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
					   ...
					   else if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[12])>90)
					   {   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
			...
			if(NowMagicNumber == MagicNumberFourteen)
			{
				...
				{   	   
				   ...
				   else
				   {
					...
					if((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[13]) > 0)
					{
						/*初始化一个超大值，该值不可能达到*/
						BuySellPosRecord[SymPos].NextModifyPos[13] = iBars(MySymbol[SymPos],0)+10000000;	
					...
					else if(((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[13]) >-50)
						&&((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[13]) <=-15)
						&&(4.5 < OneM_BoolFlag))
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
						 if(ticket <0)
					...
					else if(((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[13]) >-15)
						&&(-4.5 < BoolCrossRecord[SymPos].CrossFlag[0])
						&&(4.5 < OneM_BoolFlag))	
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
					...
					if((0.1 < BoolCrossRecord[SymPos].CrossFlag[0])
						&&(-4.5 < ThirtyM_BoolFlag)
						&&((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[13]) < -1000))					
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
						 if(ticket <0)
						 {
					...
					if((ThirtyM_StrongWeak>0.2)||(FourH_StrongWeak>0.2))
					{
						...
						/*在非多头向下的情况下一分钟120个周期，理论上应该走完了,90分钟开始监控时间控制*/
						if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[13])>120)
						{
						  ticket =OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
						...
						else if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[13])>90)
						{   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
			...
			if(NowMagicNumber == MagicNumberFifteen)
			{
				...
				{   
				   ...
				   else
				   {
					...
					if((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[14]) > 0)
					{
						
						/*初始化一个超大值，该值不可能达到*/
						BuySellPosRecord[SymPos].NextModifyPos[14] = iBars(MySymbol[SymPos],0)+10000000;	
						MinValue3 = 100000;
						for (i= 0;i < (iBars(my_symbol,0) -BoolCrossRecord[SymPos].CrossBoolPos[1]+5);i++)
						{
							if(MinValue3 > iLow(my_symbol,0,i))
					...
					else if(((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[14]) >-50)
						&&((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[14]) <=-15)
						&&(-4.5 > FiveM_BoolFlag))
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
						 if(ticket <0)
					...
					else if(((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[14]) >-15)
						&&(4.5 > BoolCrossRecord[SymPos].CrossFlag[0])
						&&(-4.5 > FiveM_BoolFlag))					
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
					...
					if((-0.1 > BoolCrossRecord[SymPos].CrossFlag[0])
						&&(4.5 > FourH_BoolFlag)
						&&((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[14]) < -1000))					
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
						 if(ticket <0)
						 {
					...
					if((FourH_StrongWeak<0.8) || (OneD_StrongWeak < 0.8))
					{
					   ...
					   /*在非多头向上的情况下，一分钟120个周期，理论上应该走完了,90周期开始监控时间控制*/
					   if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[14])>120)
					   {
						ticket =OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
					   ...
					   else if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[14])>90)
					   {   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vbid,5,Red);
			...
			if(NowMagicNumber == MagicNumberSixteen)
			{
				...
				{   	   
				   ...
				   else
				   {
					...
					if((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[15]) > 0)
					{
						/*初始化一个超大值，该值不可能达到*/
						BuySellPosRecord[SymPos].NextModifyPos[15] = iBars(MySymbol[SymPos],0)+10000000;	
					...
					else if(((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[15]) >-50)
						&&((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[15]) <=-15)
						&&(4.5 < FiveM_BoolFlag))
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
						 if(ticket <0)
					...
					else if(((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[15]) >-15)
						&&(-4.5 < BoolCrossRecord[SymPos].CrossFlag[0])
						&&(4.5 < FiveM_BoolFlag))	
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
					...
					if((0.1 < BoolCrossRecord[SymPos].CrossFlag[0])
						&&(-4.5 < FourH_BoolFlag)
						&&((iBars(my_symbol,0)-BuySellPosRecord[SymPos].NextModifyPos[15]) < -1000))					
					{
						  ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
						 if(ticket <0)
						 {
					...
					if((FourH_StrongWeak>0.2)||(OneD_StrongWeak>0.2))
					{
						...
						/*在非多头向下的情况下一分钟120个周期，理论上应该走完了,90分钟开始监控时间控制*/
						if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[15])>120)
						{
						  ticket =OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
						...
						else if((iBars(MySymbol[SymPos],0)-BuySellPosRecord[SymPos].TradeTimePos[15])>90)
						{   	   
						  if( OrderProfit()> 0)
						  {
							   ticket = OrderClose(OrderTicket(),OrderLots(),vask,5,Red);
